...
def _create_handle(rank_id: int, device_id: int):
    ...
    assert handle.join() == 0, f"rank {rank_id}: bm handle.join failed"
    return handle
...
def _rank_main(rank_id: int, device_id: int, sync: mp.Barrier):
    ...
    try:
        ...
        import_view = handle.gva_to_va(peer_gva, bm.BmMemType.LOCAL_DEVICE)
        assert import_view != 0, f"rank {rank_id}: GVA to import view conversion failed, gva=0x{peer_gva:x}"
        print(
            f"[rank {rank_id}] route peer={peer_rank} src_gva=0x{peer_gva:x} "
            f"import_view=0x{import_view:x} device={device_id}",
            flush=True,
        )
