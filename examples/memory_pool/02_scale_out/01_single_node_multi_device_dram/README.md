...
## 02 batch data copy 示例

`02_single_node_multi_device_dram_batch_data_copy.py` 与 `01_single_node_multi_device_dram.py` 的流程相同，
区别是将普通 `BigMemory.copy_data(...)` 接口替换为
`BigMemory.copy_data_batch(src_addrs, dst_addrs, sizes, count, type, flags)` 接口。

运行：

```bash
python3 02_single_node_multi_device_dram_batch_data_copy.py
```
