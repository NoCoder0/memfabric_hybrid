...
def _rank_main(rank_id: int, device_id: int, sync: mp.Barrier):
    ...
    try:
        ...
        if rank_id == RANK_0:
            ...
            assert handle.copy_data_batch([src.data_ptr()], [gva_me], [COPY_BYTES], 1, bm.BmCopyType.H2G, 0) == 0, (
                "H2G r0"
            )
        sync.wait()

        if rank_id == RANK_0:
            assert handle.copy_data_batch([gva_me], [gva_peer], [COPY_BYTES], 1, bm.BmCopyType.G2G, 0) == 0, "G2G 0→1"
        sync.wait()

        if rank_id == RANK_1:
            exp = torch.arange(COPY_BYTES // 4, dtype=torch.int32).contiguous()
            got = torch.empty(COPY_BYTES // 4, dtype=torch.int32)
            assert handle.copy_data_batch([gva_me], [got.data_ptr()], [COPY_BYTES], 1, bm.BmCopyType.G2H, 0) == 0, (
                "G2H r1"
            )
        ...
        if rank_id == RANK_1:
            assert handle.copy_data_batch([gva_me], [gva_peer], [COPY_BYTES], 1, bm.BmCopyType.G2G, 0) == 0, "G2G 1→0"
        sync.wait()
...
def main():
    ...
    print("(4/4) 02_single_node_multi_device_dram_batch_data_copy: all ranks OK", flush=True)
