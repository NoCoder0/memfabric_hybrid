...
LOCAL_RANK_SIZE = 8
RANK_START = 0
CONFIG_STORE_URL = "tcp://127.0.0.1:8570"
NIC_URL = '127.0.0.1'
AUTO_RANKING = False
ENABLE_56BITS_GVA = True

GVA_SIZE = 1024 * 1024 * 1024 * 8
MAX_GVA_SIZE = 1024 * 1024 * 1024 * 1024
BARRIER_TIMEOUT = 15
JOIN_TIMEOUT = 60
...
def child_process(
    ...
    try:
        ...
        copy_data(bm_handle, rank_id, local_ranks)
        ...
        logging.info(f'==================== rank:{rank_id}, 56-bit GVA test ok.')
...
def main_process():
    ...
    parser.add_argument(
        '--world_size',
        type=int,
        help='Number of devices used by the entire cluster.',
        required=False,
        default=WORLD_SIZE,
    )
    parser.add_argument(
        '--local_ranks',
        type=int,
        help='Number of devices used on the local node.',
        required=False,
        default=LOCAL_RANK_SIZE,
    )
    parser.add_argument(
        '--rank_start',
        type=int,
    ...
    parser.add_argument(
        ...
        required=False,
        default=CONFIG_STORE_URL,
    )
    parser.add_argument('--nic', type=str, help='device port nic', required=False, default=NIC_URL)
    parser.add_argument(
        '--auto_ranking',
        type=str_to_bool,
        ...
        default=AUTO_RANKING,
    )
    parser.add_argument(
        '--enable_56bits_gva',
        type=str_to_bool,
        help='Explicitly enable 56-bit GVA. Must be true when '
        '(max_dram_size + max_hbm_size) * world_size > 32TB; '
        'memfabric_hybrid does not auto-enable it.',
        default=ENABLE_56BITS_GVA,
    )
    ...
    barriers = [multiprocessing.Barrier(args.local_ranks, timeout=BARRIER_TIMEOUT) for _ in range(4)]
