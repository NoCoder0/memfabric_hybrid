...
#
# Prerequisite: build with `--build_hcom ON` (HOST_URMA loads libhcom via dlopen).
...
def main():
    ...
    try:
        ...
        bm_inited = True
        # register DRAM in the big memory pool using HOST_URMA protocol.
        handle = bm.create2(
            id=0,
            local_dram_size=ONE_GIB,
            max_dram_size=ONE_GIB,
            data_op_type=bm.BmDataOpType.HOST_URMA,
        )
        ...
        if bm_inited:
            bm.uninitialize(0)
        mf.uninitialize()
    logging.info("05_no_xpu_host_urma_dram_pool ok")
