...
#
# Prerequisite: build with `--build_hcom ON` (HOST_RDMA loads libhcom via dlopen).
# Tunable env vars (all optional, sensible defaults are applied when unset):
#   HCOM_RECV_DATA_SIZE, MF_HYBM_RDMA_SWAP_SPACE_SIZE
import logging
...
def main():
    ...
    try:
        ...
        bm_inited = True
        # register DRAM in the big memory pool using HOST_RDMA protocol.
        handle = bm.create2(
            id=0,
            local_dram_size=ONE_GIB,
            max_dram_size=ONE_GIB,
            data_op_type=bm.BmDataOpType.HOST_RDMA,
        )
        ...
        if bm_inited:
            bm.uninitialize(0)
        mf.uninitialize()
    logging.info("04_no_xpu_host_rdma_dram_pool ok")
