...
def _should_write_cms_config(conf_item, inputfile):
    """判断是否需要写入CMS配置"""
    return "cms" in conf_item.type.split("/") and conf_item.tag != "" and os.path.isfile(inputfile)


def _write_image_info_config(read_cfg, item_size_set, sign_file_dir, sign_tmp_path, product_delivery_path):
...
def add_bios_esbc_header(root_dir, item_size_set, sign_file_dir):
    ...
    返回：-1:失败，0：成功
    """
    bios_esbc_header_tool_path = os.path.join(root_dir, "script", "signtool", "esbc_header")
    # 检查加头工具目录是否存在
...
def _prepare_crl_file(root_dir, sign_tmp_path):
    """准备CRL文件，返回der文件路径"""
    signature_path = os.path.join(root_dir, "script", "signtool", "signature")
    bundled_crl = os.path.join(signature_path, "SWSCRL.crl")
    crl_file = os.environ.get("SIGN_CRL_FILE", bundled_crl)
    der_file = os.path.join(sign_tmp_path, "SWSCRL.der")
    if convert_der_file(crl_file, der_file) != 0:
        raise RuntimeError("failed to convert SWSCRL.crl to DER")
    return der_file


def _build_base_command(bios_tool_path):
    """构建基础命令"""
    return "{} {}".format(os.environ["HI_PYTHON"], os.path.join(bios_tool_path, "image_pack.py"))
...
def _add_cms_params(cmd: str, params: CmsSignCmdParams) -> str:
    ...
    cmd = cmd + " -cms %s.ini.p7s -ini %s.ini -crl %s -certtype 1 --addcms" % (ini_file, ini_file, params.der_file)
...
def _build_image_command(params: ImageCommandParams) -> Optional[str]:
    ...
    if params.add_sign != "true" or params.conf_item.type == "":
        ...
        cms_params = CmsSignCmdParams(
            cmd, params.input_file, params.conf_item, params.sign_path, params.input_name, params.der_file
        )
...
def _process_image_headers(params, der_file, sign_tmp_path, product_delivery_path):
    ...
    for input_name, conf_item in list(item_size_set.items()):
        input_file, sign_path = _get_image_paths(input_name, sign_file_dir, sign_tmp_path, product_delivery_path)
...
def add_bios_header(params: AddHeaderParams) -> int:
    ...
    # 准备CRL文件
    der_file = _prepare_crl_file(params.root_dir, sign_tmp_path)
...
def main(argv=None):
    ...
    bios_tool_path = os.path.join(root_dir, "script", "signtool", "image_pack")

    sgn_tool_path = os.path.join(root_dir, "script", "sign", "community_sign_build.py")
    ...
    ret_code = add_bios_header(
        AddHeaderParams(item_size_set, sign_file_dir, bios_tool_path, sgn_tool_path, root_dir, add_sign)
    )
    return ret_code
