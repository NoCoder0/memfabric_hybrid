...
def _help():
    ...
    logging.info("--------------------------------------------------------------------------------")
    logging.info("  %s: 待签名的文件路径,支持多target,各target以空格分开", "target".ljust(8))
    logging.info("====================================== END =====================================")


def get_sign_cmd(file, rootdir) -> list:
    """获取签名命令。"""
    sign_root = os.environ.get("SIGNATRUST_ROOT", "/opt/signaturst")
    bundled_crl = os.path.join(rootdir, "script/signtool/signature/SWSCRL.crl")
    sign_crl = os.environ.get("SIGN_CRL_FILE", bundled_crl)
    cmd = [
        os.path.join(sign_root, "signatrust_client"),
        "--config",
        os.path.join(sign_root, "client.toml"),
        "add",
        "--file-type",
        "p7s",
        "--key-type",
...
def _run_sign(inputfiles, rootdir):
    ...
    for file in inputfiles:
        ...
        result = subprocess.run(cmd, cwd=mypath, check=False, stdout=PIPE, stderr=STDOUT)
