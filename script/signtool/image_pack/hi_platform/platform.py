...
@dataclass
class HeaderFields:
    """按 struct 格式顺序封装镜像头字段。"""

    preamble: int
    rev0: bytes
    head_len: int
    user_len: int
    user_define_data: bytes
    code_hash: bytes
    sub_key_cert_offset: int
    sub_cert_len: int
    uw_rootkey_alg: int
    img_sign_algo: int
    root_pubkey_len: int
    ...
    e_buf: bytes
    img_offset: int
    img_sign_obj_len: int
    sign_offset: int
    sign_len: int
    code_encrypt_flag: int
    code_encrypt_algo: int
    derive_seed: bytes
    km_ireation_cnt: int
    code_encrypt_iv: bytes
    code_encrypt_tag: bytes
    code_encrypt_add: bytes
    rsv1: bytes
    h2c_enable: int
    h2c_cert_len: int
    h2c_cert_offset: int
...
def __build_header_fields(config: HeaderConfig, code_len, zero_bytes_32):
    ...
    (
        ...
        code_encrypt_tag,
        code_encrypt_add,
    ) = (512, 0xFFFFFFFF, 0x2, zero_bytes_32, 1000, zero_bytes_32[:16], zero_bytes_32[:16], zero_bytes_32[:16])
    ...
    return HeaderFields(
        ...
        code_hash=code_hash,
        sub_key_cert_offset=sub_key_cert_offset,
        sub_cert_len=sub_cert_len,
        uw_rootkey_alg=uw_rootkey_alg,
        img_sign_algo=img_sign_algo,
        root_pubkey_len=root_pubkey_len,
        n_buf=config.n_buf,
        e_buf=config.e_buf,
        img_offset=img_offset,
        img_sign_obj_len=img_sign_obj_len,
        sign_offset=sign_offset,
        sign_len=sign_len,
        code_encrypt_flag=code_encrypt_flag,
        code_encrypt_algo=code_encrypt_algo,
        derive_seed=derive_seed,
        km_ireation_cnt=km_ireation_cnt,
        code_encrypt_iv=code_encrypt_iv,
        code_encrypt_tag=code_encrypt_tag,
        code_encrypt_add=code_encrypt_add,
        rsv1=rsv1,
        h2c_enable=h2c_enable,
        h2c_cert_len=h2c_cert_len,
        h2c_cert_offset=h2c_cert_offset,
        root_pubkeyinfo=root_pubkeyinfo,
        rsv2=rsv2,
        head_magic=head_magic,
        head_hash=head_hash,
        cms_flag=cms_flag,
        code_nvcnt=code_nvcnt,
        code_tag=code_tag,
        ver_value=ver_value,
...
def __construct_header(config: HeaderConfig):
    ...
    header_fields = __build_header_fields(config, code_len, zero_bytes_32)

    s = struct.Struct("I20sII32s32sIIIII512s512sIIIIII32sI16s16s16s88sIIII20sI32s16s8s16s16s16sI52s")
    header = s.pack(*astuple(header_fields))
    return header
...
def write_extern(args, out, data_list):
    ...
    __add_magic_number_and_file_size(args, out, MagicNumberParams(args.addcms, False, code_len, before_header))
    return
