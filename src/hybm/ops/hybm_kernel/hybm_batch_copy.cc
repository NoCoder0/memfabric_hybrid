...
{
    if (param == nullptr || param->list_num == 0 || param->dst_buf_addr_list == nullptr ||
    ...
    constexpr size_t kLengthSize = sizeof(uint64_t);
    if (param->list_num > std::numeric_limits<size_t>::max() / kPointerSize ||
        param->list_num > std::numeric_limits<size_t>::max() / kLengthSize) {
        HYBM_LOGE(BM_INVALID_PARAM, "BatchCopy list byte size overflows, listNum=%u", param->list_num);
        return BM_INVALID_PARAM;
    }
    return BM_OK;
}
...
int32_t ValidateRouteHeader(const BatchCopyRouteTable *route)
{
    ...
    if (header.peerCount == 0 || header.peerCount > ock::mf::BATCH_COPY_MAX_PEER_COUNT || header.rangeCount == 0 ||
...
int32_t ValidateRouteRanges(const BatchCopyRouteTable *route)
{
    ...
    for (uint16_t index = 0; index < route->header.rangeCount; ++index) {
        ...
        auto ret = ValidateRouteRange(range, route->header.peerCount, index, rangeCounts);
        if (ret != BM_OK) {
            return ret;
        }
        if (index != 0 && range.srcGvaBegin < previousEnd) {
...
void LogRouteTableForDebug(const BatchCopyRouteTable *route)
{
    ...
    for (uint16_t index = 0; index < route->header.peerCount; ++index) {
...
int32_t ValidatePublishedRoute(const BatchCopyRouteTable *route)
{
    ...
    // Temporary ERROR-level dump for hardware debugging. Remove after route validation is complete.
...
{
    if (destination == 0 || destination > std::numeric_limits<uint64_t>::max() - length) {
                             ...
                             BatchCopyGroups &groups)
{
    ...
    if (source == 0 || source > std::numeric_limits<uint64_t>::max() - length) {
...
{
    try {
        for (uint32_t index = 0; index < param.list_num; ++index) {
...
{
    for (uint16_t peerIndex = 0; peerIndex < peerCount; ++peerIndex) {
...
int32_t SubmitPeerGroups(const BatchCopyRouteTable *route, BatchCopyGroups &groups)
{
    for (uint16_t peerIndex = 0; peerIndex < route->header.peerCount; ++peerIndex) {
        ...
        if (HybmBatchRead(&oneSide) != static_cast<uint32_t>(BM_OK)) {
            HYBM_LOGE(BM_ERROR, "HybmBatchRead failed for BatchCopy peer, peerIndex=%u itemCount=%zu", peerIndex,
                      group.lengths.size());
            return BM_ERROR;
        }
    }
