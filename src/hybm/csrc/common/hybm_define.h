...
// 全局 GVM 物理起点（offload + BM/SHM/Trans 共用的 128TB 常规 GVM 区起始地址）
constexpr uint64_t HYBM_GLOBAL_GVM_START_ADDR = 0x280040000000UL;    // 40T+1G,偏移1G规避底层问题
constexpr uint64_t HYBM_GLOBAL_GVM_START_ADDR_A5 = 0x340000000000UL; // 不同HDK版本差异有点大

// offload 独立 GVA/DVA 区段：常规 GVM 区的最前 2TB，与非 offload 模块(BM/SHM)物理隔离。
constexpr uint64_t HYBM_OFFLOAD_VA_SIZE = 2ULL * TB;                        // 2T
constexpr uint64_t HYBM_OFFLOAD_VA_START_ADDR = HYBM_GLOBAL_GVM_START_ADDR; // 常规区最前2T起点
constexpr uint64_t HYBM_OFFLOAD_VA_END_ADDR = HYBM_GLOBAL_GVM_START_ADDR + HYBM_OFFLOAD_VA_SIZE;
constexpr uint64_t HYBM_OFFLOAD_VA_START_ADDR_A5 = HYBM_GLOBAL_GVM_START_ADDR_A5;
constexpr uint64_t HYBM_OFFLOAD_VA_END_ADDR_A5 = HYBM_GLOBAL_GVM_START_ADDR_A5 + HYBM_OFFLOAD_VA_SIZE;

// GVM 搜索起点 = offload 区段结束位置。BM/SHM/Trans 在 [GVM_START_ADDR, GVM_END) first-fit，
// 无需感知 offload 区段的存在。需要判断地址是否在整个 GVM 物理范围时使用 HYBM_GLOBAL_GVM_START_ADDR。
constexpr uint64_t HYBM_GVM_START_ADDR = HYBM_OFFLOAD_VA_END_ADDR; // BM/SHM/Trans 搜索起点
constexpr uint64_t HYBM_GVM_END_ADDR = HYBM_GLOBAL_GVM_START_ADDR + HYBM_GVM_MAX_POOL_SIZE;

constexpr uint64_t HYBM_GVM_START_ADDR_A5 = HYBM_OFFLOAD_VA_END_ADDR_A5;
constexpr uint64_t HYBM_GVM_END_ADDR_A5 = HYBM_GLOBAL_GVM_START_ADDR_A5 + HYBM_GVM_MAX_POOL_SIZE;
