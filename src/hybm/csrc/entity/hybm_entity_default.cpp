...
Result MemEntityDefault::InitTagManager()
{
    ...
    if (options_.bmDataOpType & HYBM_DOP_TYPE_HOST_DEVICE_URMA) {
        compatibleInfo << localTag << ":" << HybmEntityTagInfo::GetOpTypeStr(HYBM_DOP_TYPE_HOST_DEVICE_URMA) << ":"
                       << localTag << ",";
    }
                                   ...
                                   uint32_t flags) noexcept
{
    ...
    BM_LOG_INFO("CopyData dispatch, entityId: "
                << id_ << " direction: " << direction << " srcRank: " << options.srcRankId << " destRank: "
                << options.destRankId << " src: " << VaToStr(params.src) << " dest: " << VaToStr(params.dest)
                << " size: " << params.dataSize << " stream: " << stream << " flags: " << flags);
...
int MemEntityDefault::CheckOptions(const hybm_options *options) noexcept
{
    ...
    constexpr uint32_t hcommConflictMask = HYBM_DOP_TYPE_DEVICE_URMA | HYBM_DOP_TYPE_DEVICE_UBOE;
    if ((options->bmDataOpType & HYBM_DOP_TYPE_HOST_DEVICE_URMA) != 0 &&
        (options->bmDataOpType & hcommConflictMask) != 0) {
        BM_LOG_ERROR("HOST_DEVICE_URMA cannot be enabled with DEVICE_URMA or DEVICE_UBOE, dataOpType: "
    ...
    if ((options->bmDataOpType & HYBM_DOP_TYPE_HOST_SHM) != 0) {
        ...
        constexpr uint32_t hostShmConflictMask =
            ...
            HYBM_DOP_TYPE_HOST_RDMA | HYBM_DOP_TYPE_HOST_TCP | HYBM_DOP_TYPE_HOST_URMA | HYBM_DOP_TYPE_HOST_DEVICE_URMA;
...
Result MemEntityDefault::InitTransManager()
{
    ...
    auto composeTransFlags = HYBM_DOP_TYPE_DEVICE_RDMA | HYBM_DOP_TYPE_AIV_SDMA | HYBM_DOP_TYPE_DEVICE_URMA |
                             HYBM_DOP_TYPE_DEVICE_UBOE | HYBM_DOP_TYPE_HOST_DEVICE_URMA | hostTransFlags;
...
hybm_data_op_type MemEntityDefault::CanReachDataOperators(uint32_t remoteRank) const noexcept
{
    ...
    if (options_.bmDataOpType & HYBM_DOP_TYPE_HOST_DEVICE_URMA) {
        supportDataOp |= HYBM_DOP_TYPE_HOST_DEVICE_URMA;
    }
