...
MemSegmentPtr MemSegment::Create(const MemSegmentOptions &options, int entityId)
{
    ...
    auto CONN_BASED_SEGMENT = HYBM_DOP_TYPE_DEVICE_RDMA | HYBM_DOP_TYPE_DEVICE_URMA | HYBM_DOP_TYPE_HOST_TCP;
...
Result MemSegment::RegisterMemCommon(const void *addr, uint64_t size, MemSlicePtr &slice)
{
    ...
    if (isHbm) {
        ...
        const bool needDeviceVa = (options_.dataOpType & (HYBM_DOP_TYPE_DEVICE_RDMA | HYBM_DOP_TYPE_DEVICE_URMA)) != 0U;
        if (needDeviceVa) {
