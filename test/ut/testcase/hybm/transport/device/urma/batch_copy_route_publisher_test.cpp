...
std::vector<BatchCopyRouteSource> MakeSources()
{
    ...
    first.ranges = {{0x5000U, 0x6000U, 0xA5000U}, {0x1000U, 0x2000U, 0xA1000U}};
    ...
    second.ranges = {{0x3000U, 0x4000U, 0xA3000U}};
    return {first, second};
}
...
TEST(BatchCopyRoutePublisherTest, PublishesSortedImageAndMagicLast)
{
    ...
    EXPECT_EQ(table.ranges[0].hcommVaBegin, 0xA1000U);
...
TEST(BatchCopyRoutePublisherTest, RejectsNullEndpointBeforeWritingMetadata)
{
    ApiGuard guard;
    HcommTransportManager manager;
    BatchCopyRoutePublisher publisher(MOCK_USER_DEVICE_ID, nullptr, manager);

    EXPECT_EQ(publisher.Publish(MakeSources()), BM_NOT_INITIALIZED);
...
TEST(BatchCopyRoutePublisherTest, ClearMagicFailureRetainsOwnerAndCompletion)
{
    ApiGuard guard;
    HcommTransportManager manager;
    const auto endpoint = MakeEndpoint();
    ...
    EXPECT_EQ(first.Clear(), BM_ERROR);
    EXPECT_TRUE(first.IsPublished());
    EXPECT_EQ(g_memUnregCount, 0U);
    EXPECT_EQ(second.Publish(MakeSources()), BM_BUSY);

    g_failCopyIndex = std::numeric_limits<size_t>::max();
    EXPECT_EQ(first.Clear(), BM_OK);
    EXPECT_EQ(second.Publish(MakeSources()), BM_OK);
    EXPECT_EQ(second.Clear(), BM_OK);
}

TEST(BatchCopyRoutePublisherTest, CompletionUnregisterFailureRetainsOwner)
{
    ApiGuard guard;
    HcommTransportManager manager;
    const auto endpoint = MakeEndpoint();
    ...
    ASSERT_EQ(first.Publish(MakeSources()), BM_OK);
    g_failMemUnreg = true;
    EXPECT_EQ(first.Clear(), BM_DL_FUNCTION_FAILED);
    EXPECT_FALSE(first.IsPublished());
    EXPECT_EQ(second.Publish(MakeSources()), BM_BUSY);
...
TEST(BatchCopyRoutePublisherTest, RejectsInvalidRangesBeforeAcquiringOwner)
{
    ApiGuard guard;
    HcommTransportManager manager;
    BatchCopyRoutePublisher publisher(MOCK_USER_DEVICE_ID, MakeEndpoint(), manager);
    auto sources = MakeSources();
    sources[1].ranges[0] = {0x1800U, 0x2800U, 0xA3000U};
    ...
    sources = MakeSources();
    sources[0].ranges[0].hcommVaBegin = std::numeric_limits<uint64_t>::max() - 0x10U;
    EXPECT_EQ(publisher.Publish(sources), BM_INVALID_PARAM);
    EXPECT_TRUE(g_copyEvents.empty());
}
