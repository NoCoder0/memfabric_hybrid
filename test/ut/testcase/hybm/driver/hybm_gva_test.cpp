...
TEST(HybmGvaTest, ModernMapsCompleteControlRegion)
{
    HalApiGuard guard;
    void *base = nullptr;
    void *handle = nullptr;

    EXPECT_EQ(HybmModernInitMetaGva(&base, HYBM_DEVICE_CONTROL_SIZE, &handle), BM_OK);
...
TEST(HybmGvaTest, ModernCreateFailureRollsBackReservation)
{
    HalApiGuard guard;
...
TEST(HybmGvaTest, ModernMapFailureRollsBackHandleAndReservation)
{
    HalApiGuard guard;
