...
if __name__ == "__main__":
    ...
    print(
        f"hbm_gva({hex(node0_hbm_gva)}) dram_gva({hex(node0_dram_gva)}) "
    ...
    # ============= write ================
    # LH2GH
    ret = client.bm_copy_batch([local_dram], [dram_gva_1], [size], 1, 'H2G', 0)
    assert ret == 0
    ...
    # LH2GD
    ret = client.bm_copy_batch([local_dram], [hbm_gva_1], [size], 1, 'H2G', 0)
    assert ret == 0
    ...
    # LD2GH
    ret = client.bm_copy_batch([local_hbm], [dram_gva_1], [size], 1, 'L2G', 0)
    assert ret == 0
    gva_data_sum = client.calc_gva_sum(dram_gva_1, size)
    expect_data_sum = client.calc_local_memory_sum(local_hbm)
    assert gva_data_sum == expect_data_sum
    # LD2GD
    ret = client.bm_copy_batch([local_hbm], [hbm_gva_1], [size], 1, 'L2G', 0)
    assert ret == 0
    gva_data_sum = client.calc_gva_sum(hbm_gva_1, size)
    expect_data_sum = client.calc_local_memory_sum(local_hbm)
    assert gva_data_sum == expect_data_sum
    ...
    # ============= read ================
    # GH2LH
    ret = client.bm_copy_batch([dram_gva_1], [local_dram], [size], 1, 'G2H', 0)
    assert ret == 0
    gva_data_sum = client.calc_gva_sum(dram_gva_1, size)
    expect_data_sum = client.calc_local_memory_sum(local_dram)
    assert gva_data_sum == expect_data_sum
    # GH2LD
    ret = client.bm_copy_batch([dram_gva_1], [local_hbm], [size], 1, 'G2L', 0)
    assert ret == 0
    gva_data_sum = client.calc_gva_sum(dram_gva_1, size)
    expect_data_sum = client.calc_local_memory_sum(local_hbm)
    assert gva_data_sum == expect_data_sum
    # GD2LH
    ret = client.bm_copy_batch([hbm_gva_1], [local_dram], [size], 1, 'G2H', 0)
    assert ret == 0
    gva_data_sum = client.calc_gva_sum(hbm_gva_1, size)
    expect_data_sum = client.calc_local_memory_sum(local_dram)
    assert gva_data_sum == expect_data_sum
    # GD2LD
    ret = client.bm_copy_batch([hbm_gva_1], [local_hbm], [size], 1, 'G2L', 0)
    assert ret == 0
    gva_data_sum = client.calc_gva_sum(hbm_gva_1, size)
    expect_data_sum = client.calc_local_memory_sum(local_hbm)
    assert gva_data_sum == expect_data_sum
    ...
    # ============= g2g ================
    # GH2GH
    ret = client.bm_copy_batch([dram_gva_1], [dram_gva_2], [size], 1, 'G2G', 0)
    assert ret == 0
    ...
    # GH2GD
    ret = client.bm_copy_batch([dram_gva_1], [hbm_gva_1], [size], 1, 'G2G', 0)
    assert ret == 0
    gva_data_sum1 = client.calc_gva_sum(dram_gva_1, size)
    gva_data_sum2 = client.calc_gva_sum(hbm_gva_1, size)
    assert gva_data_sum1 == expect_data_sum
    # GD2GH
    ret = client.bm_copy_batch([hbm_gva_1], [dram_gva_2], [size], 1, 'G2G', 0)
    assert ret == 0
    gva_data_sum1 = client.calc_gva_sum(dram_gva_1, size)
    gva_data_sum2 = client.calc_gva_sum(dram_gva_2, size)
    assert gva_data_sum1 == expect_data_sum
    ...
    # GD2GD
    ret = client.bm_copy_batch([hbm_gva_1], [hbm_gva_2], [size], 1, 'G2G', 0)
    assert ret == 0
