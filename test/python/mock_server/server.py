...
class MfTest(TestServer):
    ...
    def malloc_tensor(self, layer_num: int = 1, mini_block_size: int = 1024, device='cpu'):
        if mini_block_size <= 0:
            self.cli_print("mini_block_size should be positive")

        if device == "npu":
            npu_addr = self.malloc_npu_tensor(shape=(layer_num, mini_block_size))
            self.cli_print(f"tensor addr:{npu_addr.data_ptr()}")
        elif device == "cpu":
            cpu_addr = self.malloc_cpu_tensor(shape=(layer_num, mini_block_size))
            self.cli_print(f"tensor addr:{cpu_addr.data_ptr()}")
        else:
            raise RuntimeError(f"Invalid device: {device}")

    def set_device(self):
        acl.init()
        ret = acl.rt.set_device(globals_device_id)
        if ret != 0:
