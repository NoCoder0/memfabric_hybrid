...
#ifndef ZBAL_MOE_DISTRIBUTE_DISPATCH_H
#define ZBAL_MOE_DISTRIBUTE_DISPATCH_H
...
constexpr uint64_t kGmAlign = 512; // 512B alignment for stateWindowBase
...
public:
    ...
    __aicore__ inline void Init(GM_ADDR x, GM_ADDR expertIds, GM_ADDR scales, GM_ADDR shareScales, GM_ADDR shareX1Token,
                                GM_ADDR xActiveMask, GM_ADDR expandXOut, GM_ADDR shareX1Scale, GM_ADDR dynamicScalesOut,
                                GM_ADDR expandIdxOut, GM_ADDR expertTokenNumsOut, GM_ADDR sendCountsOut,
                                GM_ADDR expertTokenNums, GM_ADDR tpSendCountsOut, GM_ADDR workspaceGM, TPipe *pipe,
                                __gm__ const FusedDeepMoeTilingData *tilingData, GM_ADDR epMetaGM, GM_ADDR tpMetaGM);
...
private:
    ...
    {
        uint64_t offset = winDataSizeOffset_ + rankId * OPT_RANK_OFFSET;
        return GetZbalDataAddr(&zbalContext_[ctxIdx], rankId, offset);
    }
    ...
    GlobalTensor<uint8_t> peerMemsAddrGm_;
    uint32_t axisBS_{0};
    ...
    uint32_t preCnt_{0};
    ZbalCommContext zbalContext_[COMM_NUM];
    // loop optimization
...
__aicore__ inline void CamMoeDistributeDispatch<TemplateDispatchTypeFunc>::Init(
    ...
    GM_ADDR workspaceGM, TPipe *pipe, __gm__ const FusedDeepMoeTilingData *tilingData, GM_ADDR epMetaGM,
    GM_ADDR tpMetaGM)
{
    tpipe_ = pipe;
    aivId_ = GetBlockIdx();
    ...
    zbalContext_[COMM_EP_IDX].comm = epComm;
    zbalContext_[COMM_EP_IDX].dataWindowBase = reinterpret_cast<__gm__ uint8_t *>(epComm->myAddressExchangeGva);
    zbalContext_[COMM_EP_IDX].myRankId = epComm->myGroupRank;
    zbalContext_[COMM_EP_IDX].worldSize = epComm->groupSize;
    zbalContext_[COMM_EP_IDX].localDeviceMemSize = epComm->localDeviceMemSize;
    uint64_t totalMetaSize = epComm->sizeForExchangeAddress;
    // 512B-align to match host-side computation.
    zbalContext_[COMM_EP_IDX].dataWindowSize = (totalMetaSize * 2 / 3) & ~(kGmAlign - 1);
    zbalContext_[COMM_EP_IDX].stateWindowSize = totalMetaSize - zbalContext_[COMM_EP_IDX].dataWindowSize;
    zbalContext_[COMM_EP_IDX].stateWindowBase =
        zbalContext_[COMM_EP_IDX].dataWindowBase + zbalContext_[COMM_EP_IDX].dataWindowSize;

    // Setup TP zbal context
    auto tpComm = reinterpret_cast<__gm__ CommGroupInfo *>(tpMetaGM);
    zbalContext_[COMM_TP_IDX].comm = tpComm;
    zbalContext_[COMM_TP_IDX].dataWindowBase = reinterpret_cast<__gm__ uint8_t *>(tpComm->myAddressExchangeGva);
    zbalContext_[COMM_TP_IDX].myRankId = tpComm->myGroupRank;
    zbalContext_[COMM_TP_IDX].worldSize = tpComm->groupSize;
    zbalContext_[COMM_TP_IDX].localDeviceMemSize = tpComm->localDeviceMemSize;
    totalMetaSize = tpComm->sizeForExchangeAddress;
    // 512B-align to match host-side computation.
    zbalContext_[COMM_TP_IDX].dataWindowSize = (totalMetaSize * 2 / 3) & ~(kGmAlign - 1);
    zbalContext_[COMM_TP_IDX].stateWindowSize = totalMetaSize - zbalContext_[COMM_TP_IDX].dataWindowSize;
    zbalContext_[COMM_TP_IDX].stateWindowBase =
        zbalContext_[COMM_TP_IDX].dataWindowBase + zbalContext_[COMM_TP_IDX].dataWindowSize;

    statusDataSpaceGm = (GM_ADDR)(zbalContext_[COMM_EP_IDX].stateWindowBase);
...
CamMoeDistributeDispatch<TemplateDispatchTypeFunc>::FillExpertCountByRowRange(uint32_t startTokenRow,
                                                                              uint32_t endTokenRow)
{
    for (int tokenIndex = startTokenRow * axisK_; tokenIndex < endTokenRow * axisK_; ++tokenIndex) {
        ...
        expertCountTensor_(tokenIndex) = (int32_t)tableLocalTensor_(row * moeExpertRankNumAligned_ + expertId);
    }
}

template<TemplateDispatchTypeClass>
__aicore__ inline void
CamMoeDistributeDispatch<TemplateDispatchTypeFunc>::CountAndPrepareStatusWithAivOpt(uint32_t expertIdsCnt)
{
    ...
    if (aivId_ < moeUsedAivNum_) {
        ...
        for (int row = static_cast<int>(startTokenRow); row < static_cast<int>(endTokenRow); ++row) {
            expandIdxGMTensor.SetGlobalBuffer((__gm__ int32_t *)(expandIdxOutGM_ + row * axisK_ * sizeof(uint32_t)));
    ...
    for (int32_t tmpExpertId = static_cast<int32_t>(preStartExpertId);
        ...
        statusTensor_(tmpExpertId * INT32_NUM_PER_BLOCK + 1) = (int32_t)sendCountLocalTensor_(tmpExpertId);
    }
}
...
__aicore__ inline void CamMoeDistributeDispatch<TemplateDispatchTypeFunc>::SetStatus()
{
    ...
    for (uint32_t rankIndex = startExpertId_; rankIndex < endExpertId_; ++rankIndex) {
        ...
        if (moeExpertNumPerRank_ > 1) {
            ...
            offset = (epRankId_ + rankIndex % moeExpertNumPerRank_ * epWorldSize_) * stateOffset_;
        }
...
template<TemplateDispatchTypeClass>
__aicore__ inline void
CamMoeDistributeDispatch<TemplateDispatchTypeFunc>::QuantProcess(uint32_t expertIndex,
                                                                 const GlobalTensor<float> &scalesGMTensor)
{
...
__aicore__ inline void CamMoeDistributeDispatch<TemplateDispatchTypeFunc>::AllGatherSetStatusAndWait()
{
    ...
    SyncFunc<AscendC::HardEvent::S_MTE3>();
    DataCopy<float>(tpwindowInstatusFp32Tensor_, statusFp32Tensor_[aivId_ * 8], UB_ALIGN);
    SyncFunc<AscendC::HardEvent::MTE3_S>();
    float sumOfFlag = static_cast<float>(-1.0);
    rankGM = (__gm__ uint8_t *)(GetWindStateAddrByRankId(COMM_TP_IDX, tpRankId_) + stateOffset_ * aivId_);
    tpwindowInstatusFp32Tensor_.SetGlobalBuffer((__gm__ float *)(rankGM));
...
__aicore__ inline void CamMoeDistributeDispatch<TemplateDispatchTypeFunc>::UpdataTokenNumsOut()
{
    ...
    if (moeExpertNumPerRank_ > 1) {
        ...
        DataCacheCleanAndInvalid<int64_t, CacheLine::SINGLE_CACHE_LINE, DcciDst::CACHELINE_OUT>(expertTokenNumsTensor);
    }
    if constexpr (IsNeedAllgater) {
...
#endif // ZBAL_MOE_DISTRIBUTE_DISPATCH_H
