...
CATLASS_DEVICE Callback MakeCallbackWithCall(Func *func)
{
    Callback callback;
    callback.func = func;
    callback.caller = [](void const *f) { static_cast<Func const *>(f)->Call(); };
    return callback;
}

template<typename Func>
CATLASS_DEVICE Callback MakeCallbackWithCall2(Func *func)
{
    Callback callback;
    callback.func = func;
    callback.caller = [](void const *f) { static_cast<Func const *>(f)->Call2(); };
    return callback;
}
...
public:
    ...
    struct Params {
        ...
        Params(GemmCoord problemShape_, uint32_t problemCount_, GM_ADDR ptrGroupList_, GM_ADDR ptrA_, LayoutA layoutA_,
               ...
               LayoutB sharedLayoutB_, LayoutPerTokenScale sharedLayoutPerTokenScale_, LayoutD sharedLayoutD_,
               GM_ADDR ptrWorkspace_, GM_ADDR epMetaGM_, void *combiner_)
            : problemShape(problemShape_), problemCount(problemCount_),
              ...
              layoutPerTokenScale(layoutPerTokenScale_), ptrD(reinterpret_cast<__gm__ ElementD *>(ptrD_)),
              layoutD(layoutD_), batchSize(batchSize_), sharedGmm2ProblemShape(sharedGmm2ProblemShape_),
              ...
              sharedLayoutA(sharedLayoutA_), sharedLayoutB(sharedLayoutB_),
              sharedLayoutPerTokenScale(sharedLayoutPerTokenScale_), sharedLayoutD(sharedLayoutD_),
              ptrWorkspace(ptrWorkspace_), epMetaGM(epMetaGM_), combiner(combiner_)
        {}
    };
    ...
    {
        // Setup zbal context and derive syncGmAddr from zbal state window (replaces HCCL GetHcclContext)
        ...
        // 512B-align to match host-side computation so stateWindowBase
        // is at the same address as host expects.
        zbalContext_.dataWindowSize = (totalMetaSize * 2 / 3) & ~(GMM2::kGmAlign - 1);
        ...
        syncGmAddr = (GM_ADDR)(zbalContext_.stateWindowBase) + GMM2::SOFT_SYNC_OFFSET +
        ...
        // Only wrap as ListTensorDesc when TENSOR_LIST flag is actually set.
    ...
    {
        // Setup zbal context and derive syncGmAddr from zbal state window
        auto epComm = reinterpret_cast<__gm__ CommGroupInfo *>(params.epMetaGM);
        zbalContext_.dataWindowBase = reinterpret_cast<__gm__ uint8_t *>(epComm->myAddressExchangeGva);
        uint64_t totalMetaSize = epComm->sizeForExchangeAddress;
        // 512B-align to match host-side computation so stateWindowBase
        ...
        // Exchange combine workspace GVAs through state window before the
        // epilogue writes to remote ranks' workspace via DoCombineSend.
        // PublishAndWaitCombineGva uses TPipe buffers — init pipe first.
        resource.pipe.Init();
        combiner->TPipeSet(&resource.pipe);
        combiner->PublishCombineGva();
        ...
        do {
            ...
            if constexpr (EXEC_FLAG & EXEC_FLAG_SHARED_EXPERT) {
                if (AscendC::GetSubBlockIdx() == 0) {
                    ...
                    LayoutScale layoutScale = params.layoutScale;
                    LayoutPerTokenScale layoutPerTokenScale = params.sharedLayoutPerTokenScale.GetTileLayout(
                    ...
                    EpilogueParams epilogueParams{
                        params.ptrSharedScale, layoutScale,       params.ptrSharedPtrPerTokenScale,
                        layoutPerTokenScale,   params.ptrSharedD, layoutD};
...
private:
            ...
            while (true) {
                if constexpr (g_coreType == AscendC::AIC) {
                    if (waitOffset[0] != -1) {
                        __asm__ __volatile__("");
                        AscendC::DataCacheCleanAndInvalid<uint8_t, AscendC::CacheLine::SINGLE_CACHE_LINE,
                                                          AscendC::DcciDst::CACHELINE_OUT>(global[waitOffset[0]]);
                        __asm__ __volatile__("");
                    ...
                    if (waitOffset[1] != -1) {
                        __asm__ __volatile__("");
                        AscendC::DataCacheCleanAndInvalid<uint8_t, AscendC::CacheLine::SINGLE_CACHE_LINE,
                                                          AscendC::DcciDst::CACHELINE_OUT>(global[waitOffset[1]]);
                        __asm__ __volatile__("");
    ...
    AscendC::GlobalTensor<uint8_t> epWinContext_;
