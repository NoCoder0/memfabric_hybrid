...
*/
#ifndef FUSED_DEEP_MOE_GEMM_KERNEL_GROUPED_MATMUL_SLICE_M_PER_TOKEN_DEQUANT_SWIGLU_QUANT_MULTISTAGE_WORKSPACE_H
#define FUSED_DEEP_MOE_GEMM_KERNEL_GROUPED_MATMUL_SLICE_M_PER_TOKEN_DEQUANT_SWIGLU_QUANT_MULTISTAGE_WORKSPACE_H
...
constexpr uint32_t TOKEN_EXTRA_SPACE = 512;          // used by QuantToken local buffer only
constexpr uint32_t QUANT_SCALE_SIZE = sizeof(float); // 4B dequant scale per token (no tokenFlag needed)
// Pull-mode dispatch metadata region in zbal state window (128 KB ~ 256 KB).
// Layout (each per-rank slot = INT32_COUNT_PER_BLOCK * sizeof(uint32_t) = 32B,
//  because cross-NPU state window writes require 32B-aligned DataCopy blocks):
//   DISPATCH_FLAGS_OFFSET           : "data ready" flags  [epRankSize * 32 B]
//   dispatchSlotsOffset_            : per-rank metadata slots (computed dynamically)
// Each flag slot: [flag:4B][padding:28B]; only slot[0] carries the flag value.
constexpr uint32_t INT32_COUNT_PER_BLOCK = 8;
constexpr uint64_t DISPATCH_FLAGS_OFFSET = 128 * 1024; // "data ready" flags (512 B for 16 ranks)
// Offset in zbal data window where quantized tokens are stored during dispatch.
// Data window is symmetric memory; each rank writes contiguously-by-expert.
constexpr uint64_t DISPATCH_QUANT_DATA_OFFSET = 0;
...
// VECIN UB offset for cross-NPU DataCopyPad reads (same pattern as CpGM2GM).
constexpr uint32_t VECIN_READ_OFFSET = 32 * 1024;
// Max tokens to batch in one VECIN read.  Tokens for the same (rank, expert)
// are contiguous in the remote quant buffer.  Batching reduces HCCS round-trips.
// VECIN ≈ 64 KB; with VECIN_READ_OFFSET = 32 KB, ~32 KB remains → ~4 tokens
// at quantStride ≈ 7 KB (h = 7168).
constexpr uint32_t MAX_VECIN_TOKENS = 4;
...
public:
    ...
    struct Params {
        ...
        Params(__gm__ ElementInput *ptrInput_, LayoutInput const &layoutInput_,
            ...
            : ptrInput(ptrInput_), layoutInput(layoutInput_), ptrDequantScale(ptrQuantScale_),
              layoutDequantScale(layoutQuantScale_), ptrOutput(ptrOutput_), layoutOutput(layoutOutput_),
              tileRow(tileRow_), tileColumn(tileColumn_)
        {}
    };
    ...
    void operator()(MatrixCoord const &blockShape, MatrixCoord const &blockCoord, MatrixCoord const &actualBlockShape)
    {
        ...
        for (uint32_t loopIdx = subblockIdx; loopIdx < tileLoops; loopIdx += subblockNum) {
            ...
            copyGmToUbInput(ubInputRightHalf, gmTileInput[params.layoutInput.shape(1) >> 1], layoutUbInput,
                            layoutGmTileInput);
...
__aicore__ inline static void EncreaseSyncFlag(__gm__ uint8_t *flagAddr, uint8_t idx)
{
    ...
    if constexpr (g_coreType == AscendC::AIV) {
        global.SetGlobalBuffer(flagAddr + idx * SOFT_SYNC_SPACE_SIZE +
                               AscendC::GetSubBlockIdx() * (SOFT_SYNC_SPACE_SIZE / SUB_AIV_NUM));
    } else {
        global.SetGlobalBuffer(flagAddr + idx * SOFT_SYNC_SPACE_SIZE);
    }
...
// Like CheckSyncFlag but only polls a single subBlock's flag.
// Used by AIC when CompCore runs on send cores (subBlock 1) only.
__aicore__ inline static void CheckSyncFlagSingle(__gm__ uint8_t *flagAddr, uint8_t idx, uint32_t target,
                                                  uint32_t subBlock)
{
    AscendC::PipeBarrier<PIPE_ALL>();
    AscendC::GlobalTensor<uint8_t> global;
    global.SetGlobalBuffer(flagAddr + idx * SOFT_SYNC_SPACE_SIZE + subBlock * (SOFT_SYNC_SPACE_SIZE / SUB_AIV_NUM));
    while (true) {
        __asm__ __volatile__("");
        AscendC::DataCacheCleanAndInvalid<uint8_t, AscendC::CacheLine::SINGLE_CACHE_LINE,
                                          AscendC::DcciDst::CACHELINE_OUT>(global);
...
public:
    ...
    struct Params {
        ...
        GM_ADDR gmQuantWorkspace; // local workspace for quantized tokens (pull-mode dispatch)
        ...
        Params(GemmCoord problemShape_, uint32_t problemCount_, GM_ADDR ptrGroupList_, GM_ADDR ptrA_,
               ...
               LayoutB const &layoutB_, GM_ADDR ptrShareScale_, LayoutScale const &layoutShareScale_, GM_ADDR ptrScale_,
               ...
               GM_ADDR gmSwigluOut_, GM_ADDR epMetaGM_, GM_ADDR tpMetaGM_,
               const __gm__ FusedDeepMoeInfo &disGmmDeqSwigluQuantGmmDeqComInfo)
            : problemShape(problemShape_), problemCount(problemCount_),
              ...
              layoutShareScale(layoutShareScale_), ptrScale(reinterpret_cast<__gm__ ElementScale *>(ptrScale_)),
              ...
              layoutDequantScale(layoutDequantScale_), ptrWorkspace(ptrWorkspace_), gmX(gmX_),
              gmMoeSmoothScales(gmMoeSmoothScales_), gmShareSmoothScales(gmShareSmoothScales_),
              gmexpertIds(gmexpertIds_), gmExpandIdx(gmExpandIdx_), gmEpSendCount(gmEpSendCount_),
              gmExpertTokenNums(gmExpertTokenNums_), gmXActiveMask(gmXActiveMask_), gmResvered(gmResvered_),
              gmShareX1(gmShareX1_), gmShareX1Scale(gmShareX1Scale_), gmShareSwigluOut(gmShareSwigluOut_),
              gmShareX2(gmShareX2_), layoutShareOutput(layoutShareOutput_), gmShareX2Scale(gmShareX2Scale_),
              gmSwigluOut(gmSwigluOut_), epMetaGM(epMetaGM_), tpMetaGM(tpMetaGM_),
              ...
              globalBs(disGmmDeqSwigluQuantGmmDeqComInfo.globalBs), bs(disGmmDeqSwigluQuantGmmDeqComInfo.bs),
              topK(disGmmDeqSwigluQuantGmmDeqComInfo.k), tokenLen(disGmmDeqSwigluQuantGmmDeqComInfo.h),
    ...
    {
        uint64_t offset = dataState * WIN_STATE_OFFSET;
        return GetZbalStateAddr(&zbalContext_, rankId, offset);
    }

    __aicore__ inline GM_ADDR GetWindAddrByRankId(int64_t rankId)
    {
        uint64_t offset = winDataSizeOffset + rankId * OPT_RANK_OFFSET;
        return GetZbalDataAddr(&zbalContext_, rankId, offset);
    }
    ...
    CATLASS_DEVICE void operator()<AscendC::AIC>(Params const &params)
    {
        ...
        // Setup zbal context from params (replaces HCCL GetHcclContext)
        auto epComm = reinterpret_cast<__gm__ CommGroupInfo *>(params.epMetaGM);
        zbalContext_.comm = epComm;
        zbalContext_.dataWindowBase = reinterpret_cast<__gm__ uint8_t *>(epComm->myAddressExchangeGva);
        zbalContext_.myRankId = epComm->myGroupRank;
        zbalContext_.worldSize = epComm->groupSize;
        zbalContext_.localDeviceMemSize = epComm->localDeviceMemSize;
        uint64_t totalMetaSize = epComm->sizeForExchangeAddress;
        constexpr uint64_t kGmAlign = 512;
        if constexpr (EXEC_FLAG & EXEC_FLAG_DEEP_FUSE) {
            zbalContext_.dataWindowSize = 0;
            zbalContext_.stateWindowSize = totalMetaSize;
        } else {
            zbalContext_.dataWindowSize = (totalMetaSize * 2 / 3) & ~(kGmAlign - 1);
            zbalContext_.stateWindowSize = totalMetaSize - zbalContext_.dataWindowSize;
        }
        zbalContext_.stateWindowBase = zbalContext_.dataWindowBase + zbalContext_.dataWindowSize;
        statusDataSpaceGm = (GM_ADDR)(zbalContext_.stateWindowBase);
        ...
        // ZBAL: ptrB is a raw data pointer (not a ListTensorDesc from CANN opbuild).
        // Only wrap as ListTensorDesc when TENSOR_LIST flag is set.
        AscendC::ListTensorDesc gmBlistTensorDesc;
        if constexpr (EXEC_FLAG & EXEC_FLAG_TENSOR_LIST) {
            gmBlistTensorDesc = AscendC::ListTensorDesc(reinterpret_cast<__gm__ void *>(params.ptrB));
        }
            ...
            for (uint32_t loopIdx = startLoopIdx; loopIdx < coreLoops; loopIdx += aicNum) {
                ...
                if (stageUsed == WORKSPACE_STAGES) {
                    aicWaitFunc1 = {statusDataSpaceGm + SOFT_SYNC_OFFSET, static_cast<uint8_t>(AscendC::GetBlockIdx()),
                                    target, true}; // checkSubBlock1Only=true
                    target += 1;
        ...
        // ZBAL: gmB was already set from params.ptrB in initialization above.
        // Do NOT use gmBlistTensorDesc.GetDataPtr(0) — ptrB is raw data, not a ListTensorDesc.
        ...
        for (uint32_t groupIdx = 0; groupIdx < localExpertNum; ++groupIdx) {
            if constexpr (EXEC_FLAG & EXEC_FLAG_TENSOR_LIST) {
                gmB.SetGlobalBuffer(
                    reinterpret_cast<__gm__ ElementB *>(gmBlistTensorDesc.GetDataPtr<int32_t>(groupIdx)));
            }
    ...
    void TokenActiveMaskCal(GM_ADDR gmXActiveMask, int64_t ubOffset)
    {
        ...
        AscendC::LocalTensor<bool> maskInputTensor = (resource.ubBuf.template GetBufferByByte<bool>(subUbOffset));
        ...
        AscendC::LocalTensor<half> maskTmpTensor = (resource.ubBuf.template GetBufferByByte<half>(subUbOffset));
        subUbOffset += CEIL_UP(axisBS * sizeof(half));
        AscendC::LocalTensor<half> sumOutTensor = (resource.ubBuf.template GetBufferByByte<half>(subUbOffset));
    ...
    CATLASS_DEVICE void SendCoreFunc(GM_ADDR gmX, GM_ADDR gmExpertIds, GM_ADDR gmMoeSmoothScales, GM_ADDR gmX1,
                                     GM_ADDR gmX1Scale, GM_ADDR gmExpandIdx, GM_ADDR gmXActiveMask)
    {
        ubOffset = 0;
    ...
    void RecvCount(int64_t ubOffset)
    {
        ...
        AscendC::LocalTensor<uint32_t> gatherTmpTensor =
            (resource.ubBuf.template GetBufferByByte<uint32_t>(subUbOffset));
        subUbOffset += CEIL_UP(UB_BLOCK_SIZE);
                   ...
                   uint32_t startRankId, uint32_t endRankId, uint32_t recvRankNumPerCore, int64_t ubOffset)
    {
        ...
        for (uint32_t index = startRankId; index < endRankId; index++) {
            ...
            if (i > 0) {
                gatherMaskOutCountTensor.SetValue(i, gatherMaskOutCountTensor.GetValue(i - 1) +
                                                         gatherMaskOutCountTensor.GetValue(index));
            }
        ...
        while (curRecvTokenCount < targetTokenCount) {
            for (uint32_t index = startRankId; index < endRankId; index++) {
                ...
                for (uint32_t j = curTokenIdx[i]; j < countPerRank[i]; j++) {
                    ...
                    expandXOutGlobal.SetGlobalBuffer((__gm__ int8_t *)(gmX1) + (beginIdx + j) * tokenLength,
                                                     tokenLength);
        ...
        AscendC::SetFlag<AscendC::HardEvent::S_MTE3>(0);
        AscendC::WaitFlag<AscendC::HardEvent::S_MTE3>(0);
        AscendC::DataCopyExtParams dataCopyOutParams = {1U, static_cast<uint32_t>(recvRankNumPerCore * sizeof(int32_t)),
                                                        0U, 0U, 0U};
    ...
    void CompCoreFunc(GM_ADDR gmCVSwapBuff, __gm__ ElementScale *gmShareMm1Scale, __gm__ ElementScale *gmScale,
                      ...
                      LayoutPerTokenScale wholeLayoutPerTokenScale, bool allTilesMode = false)
    {
        uint32_t coreNumPerGroup = recvCoreNum;
        ...
        auto layoutC = layout::RowMajor{L1TileShape::M * aiCoreGroupNum * WORKSPACE_STAGES, L1TileShape::N};
        {
            ...
            if constexpr (EXEC_FLAG & EXEC_FLAG_SHARED_EXPERT) {
                ...
                EpilogueParams epilogueParams{gmShareMm1Scale,     layoutShareScale,    gmShareTokenScale,
                                              layoutPerTokenScale, gmShareSwigluOutput, layoutD};
                ...
                for (uint32_t loopIdx = startLoopIdx; loopIdx < coreLoops; loopIdx += aiCoreGroupNum) {
                    ...
                    CheckSyncFlag(statusDataSpaceGm + SOFT_SYNC_OFFSET, static_cast<uint8_t>(compCoreNum + compCoreIdx),
                                  target);
                    target += 1;
                    blockEpilogue(blockShapeMNK, blockCoordMNK, actualBlockShapeMNK, gmBlockC, layoutBlockC, true,
                                  Callback{}, allTilesMode);
            ...
            // ZBAL: gmScale is a raw data pointer (not a ListTensorDesc from CANN opbuild).
            // Only wrap as ListTensorDesc when TENSOR_LIST flag is set.
            ...
            for (uint32_t groupIdx = 0; groupIdx < localExpertNum; ++groupIdx) {
                ...
                if constexpr (EXEC_FLAG & EXEC_FLAG_TENSOR_LIST) {
                    gmScalePtr =
                        reinterpret_cast<__gm__ ElementScale *>(gmScaleListTensor.GetDataPtr<int32_t>(groupIdx));
                    epilogueParams = EpilogueParams{gmScalePtr,
                                                    layoutScale,
                    ...
                    epilogueParams = EpilogueParams{gmScalePtr + gmGroupOffsetScale,           layoutScale,
    ...
    void AivInitParams(Params const &params)
    {
        ...
        scaleParamPad = TOKEN_EXTRA_SPACE;                  // 512B used only by QuantToken local buffer
        hCommuSize = hOutSize + scaleParamPad;              // local buffer size (unchanged for QuantToken compat)
        quantStride = CEIL_UP(hOutSize + QUANT_SCALE_SIZE); // 32B aligned for DataCopy
        totalExperts = epRankSize * moeExpertNumPerRank;
        // Pull-mode dispatch: compute metadata slot layout dynamically.
        // State window usage (within [128KB, 256KB)):
        //   DISPATCH_FLAGS_OFFSET (128K) → "data ready" flags [epRankSize * 32B]
        //   dispatchSlotsOffset_          → per-rank metadata slots [epRankSize * metaSlotSize_]
        // Each flag slot uses INT32_COUNT_PER_BLOCK * sizeof(uint32_t) = 32B for
        // cross-NPU 32B-aligned DataCopy compatibility.
        dispatchFlagsSize_ = epRankSize * INT32_COUNT_PER_BLOCK * sizeof(uint32_t);
        dispatchSlotsOffset_ = DISPATCH_FLAGS_OFFSET + dispatchFlagsSize_;
        // Each per-rank metadata slot holds raw per-expert counts only,
        // rounded up to 32B for cross-NPU DataCopy alignment.
        // quantDelta (= DISPATCH_QUANT_DATA_OFFSET) is the data-window offset
        // of quantized tokens; each rank computes it locally.
        // +1 for completion flag embedded at element totalExperts.
        metaSlotSize_ = CEIL_UP((totalExperts + 3) * sizeof(uint32_t)); // +1 flag +2 GVA
        // Total: dispatchSlotsOffset_ + epRankSize * metaSlotSize_ < SELF_STATE_OFFSET (256KB)
        axisHCommu = hCommuSize / sizeof(int8_t); // local buffer size in int8 units
        axisBS = params.bs;
    ...
    void AivInitState()
    {
        ...
        // Dispatch push and combine push share dataWin via dataState flip.
        // quantDataOffset_ must use the same base stride as the push-mode
        // dispatch so the two regions never overlap.  winDataSizeOffset
        // provides per-(rank,expert) slots large enough for all quant data
        // (maxBs * h bytes per expert, worst-case > quantStride per token).
        quantDataOffset_ = winDataSizeOffset;
        ...
        // metaCompleteFlag: flipped per kernel call, same pattern as sumTarget
        if (state == 0) {
            metaCompleteFlag = 0x3F800000; // 1.0f
        } else {
            metaCompleteFlag = 0;
        }
    }
    ...
    // === Pull-mode dispatch (DEEP_FUSE): LocalQuant → PublishMeta → RemoteRead ===

    // Quantize all tokens, partitioned by EXPERT across AIV cores.
    // Writes to gmQuantWorkspace_ contiguously by expert: [exp0 tokens...][exp1 tokens...]
    CATLASS_DEVICE void LocalQuantFunc(GM_ADDR gmX, GM_ADDR gmExpertIds, GM_ADDR gmMoeSmoothScales,
                                       GM_ADDR gmEpSendCount, GM_ADDR gmExpandIdx, GM_ADDR gmQuantWs)
    {
        // Expert range for this core: partition totalExperts across aivNum cores.
        uint32_t expertPerCore = totalExperts / aivNum;
        uint32_t remExp = totalExperts % aivNum;
        uint32_t startExpert = expertPerCore * aivIdx;
        if (aivIdx < remExp) {
            expertPerCore += 1;
            startExpert += aivIdx;
        } else {
        ...
        uint32_t endExpert = startExpert + expertPerCore;
        if (startExpert >= totalExperts) {
            AscendC::PipeBarrier<PIPE_ALL>();
            return;
        }

        // gmEpSendCount is written directly per expert range (no zeroing
        // or atomic-add needed — each expert is owned by exactly one core).

        // Load ALL expert IDs into member tensor (used by CalExpandxIdx).
        AscendC::GlobalTensor<int32_t> eidGm;
        eidGm.SetGlobalBuffer((__gm__ int32_t *)gmExpertIds);
        AscendC::DataCopyExtParams cpAll = {1, static_cast<uint32_t>(expertIdsCnt * sizeof(int32_t)), 0, 0, 0};
        AscendC::DataCopyPadExtParams<int32_t> padAll{false, 0U, 0U, 0U};
        expertIdsTensor_ = resource.ubBuf.template GetBufferByByte<int32_t>(0);
        AscendC::DataCopyPad(expertIdsTensor_, eidGm, cpAll, padAll);
        AscendC::SetFlag<AscendC::HardEvent::MTE2_S>(0);
        AscendC::WaitFlag<AscendC::HardEvent::MTE2_S>(0);
        uint32_t eidUbAllSize = CEIL_UP(expertIdsCnt * sizeof(int32_t));

        // Phase 1: count tokens for THIS core's experts + atomic-add to gmEpSendCount.
        AscendC::LocalTensor<uint32_t> ubCnt = resource.ubBuf.template GetBufferByByte<uint32_t>(eidUbAllSize);
        // DataCopy requires calCount*sizeof(T) 32B-aligned — pad to multiple of 8.
        uint32_t alignedExperts = CEIL(totalExperts, INT32_COUNT_PER_BLOCK) * INT32_COUNT_PER_BLOCK;
        AscendC::Duplicate(ubCnt, (uint32_t)0, alignedExperts);
        AscendC::SetFlag<AscendC::HardEvent::V_S>(0);
        AscendC::WaitFlag<AscendC::HardEvent::V_S>(0);
        for (uint32_t t = 0; t < expertIdsCnt; t++) {
            int32_t eid = expertIdsTensor_.GetValue(t);
            if (eid < 0 || (uint32_t)eid < startExpert || (uint32_t)eid >= endExpert)
                continue;
            ubCnt.SetValue(eid, ubCnt.GetValue(eid) + 1);
        }
        AscendC::SetFlag<AscendC::HardEvent::S_MTE3>(0);
        AscendC::WaitFlag<AscendC::HardEvent::S_MTE3>(0);

        // Direct-write per-core counts to disjoint expert ranges.
        // Each expert is owned by exactly one core, so no atomic-add needed.
        // Copy counts to a 32B-aligned temp buffer first — ubCntI32[startExpert]
        // may not be 32B-aligned when startExpert is not a multiple of 8.
        // Push-mode avoids this because its TPipe-allocated sendCountLocalTensor_
        // rows are naturally 32B-aligned.
        AscendC::GlobalTensor<int32_t> epSendGm;
        epSendGm.SetGlobalBuffer((__gm__ int32_t *)gmEpSendCount);
        {
            int64_t tmpOff = eidUbAllSize + alignedExperts * sizeof(uint32_t);
            AscendC::LocalTensor<int32_t> tmpUb = resource.ubBuf.template GetBufferByByte<int32_t>(tmpOff);
            for (uint32_t i = 0; i < expertPerCore; i++) {
                tmpUb.SetValue(i, ubCnt.GetValue(startExpert + i));
            }
            AscendC::DataCopyExtParams cp = {1, static_cast<uint32_t>(expertPerCore * sizeof(uint32_t)), 0, 0, 0};
            AscendC::DataCopyPad(epSendGm[startExpert], tmpUb, cp);
        }

        // Barrier: all per-core writes complete before reading gmEpSendCount.
        AscendC::PipeBarrier<PIPE_ALL>();
        AscendC::SyncAll<true>();

        // Read gmEpSendCount → expBaseUb → global prefix sum (across all ranks).
        // Must match RemoteReadFunc's global byteOff computation.
        int64_t qUbOff = eidUbAllSize;
        AscendC::LocalTensor<int32_t> expBaseUb = resource.ubBuf.template GetBufferByByte<int32_t>(qUbOff);
        qUbOff += alignedExperts * sizeof(uint32_t);
        AscendC::DataCopy(expBaseUb, epSendGm, alignedExperts);
        AscendC::SetFlag<AscendC::HardEvent::MTE2_S>(0);
        AscendC::WaitFlag<AscendC::HardEvent::MTE2_S>(0);
        {
            uint32_t run = 0;
            for (uint32_t e = 0; e < totalExperts; e++) {
                uint32_t cnt = expBaseUb.GetValue(e);
                expBaseUb.SetValue(e, run);
                run += cnt;
            }
        }

        // Phase 2: token-based quant + write (like original SendToMoeExprt).
        // Contiguous token range → contiguous expandIdx buffer → single DataCopyPad.
        uint32_t tokPerCore = expertIdsCnt / aivNum;
        uint32_t rem = expertIdsCnt % aivNum;
        uint32_t sTok = tokPerCore * aivIdx;
        if (aivIdx < rem) {
            tokPerCore += 1;
            sTok += aivIdx;
        } else {
        ...
        AscendC::GlobalTensor<XType> srcXGm;
        srcXGm.SetGlobalBuffer((__gm__ XType *)gmX);
        // Use dataState offset to avoid overlap with combine push region.
        AscendC::GlobalTensor<int8_t> qOutGm;
        qOutGm.SetGlobalBuffer((__gm__ int8_t *)gmQuantWs);
        AscendC::GlobalTensor<int32_t> expIdxGm;
        expIdxGm.SetGlobalBuffer((__gm__ int32_t *)gmExpandIdx);

        // UB layout for quant double-buffer.
        AscendC::LocalTensor<XType> xInT[BUFFER_NUM];
        AscendC::LocalTensor<int8_t> yOutT[BUFFER_NUM];
        xInT[0] = resource.ubBuf.template GetBufferByByte<XType>(qUbOff);
        qUbOff += CEIL_UP(tokenLength * sizeof(XType));
        xInT[1] = resource.ubBuf.template GetBufferByByte<XType>(qUbOff);
        qUbOff += CEIL_UP(tokenLength * sizeof(XType));
        yOutT[0] = resource.ubBuf.template GetBufferByByte<int8_t>(qUbOff);
        qUbOff += CEIL_UP(axisHCommu * sizeof(int8_t));
        yOutT[1] = resource.ubBuf.template GetBufferByByte<int8_t>(qUbOff);
        qUbOff += CEIL_UP(axisHCommu * sizeof(int8_t));

        // Contiguous expandIdx buffer for this core's token range.
        // Zero-init: tokens with dstEid<0 are never written in the loop
        // below but the whole buffer is flushed to gmExpandIdx at the end.
        AscendC::LocalTensor<int32_t> expIdxUb = resource.ubBuf.template GetBufferByByte<int32_t>(qUbOff);
        qUbOff += CEIL_UP(tokPerCore * sizeof(int32_t));
        AscendC::Duplicate(expIdxUb, (int32_t)0, tokPerCore);
        AscendC::SetFlag<AscendC::HardEvent::V_S>(0);
        AscendC::WaitFlag<AscendC::HardEvent::V_S>(0);

        AscendC::SetDeqScale((half)1.0f);
        AscendC::SetFlag<AscendC::HardEvent::MTE3_MTE2>(0);
        AscendC::SetFlag<AscendC::HardEvent::MTE3_MTE2>(1);
        AscendC::SetFlag<AscendC::HardEvent::V_MTE2>(0);
        AscendC::SetFlag<AscendC::HardEvent::V_MTE2>(1);

        AscendC::GlobalTensor<float> smoothScGm;
        if constexpr (EXEC_FLAG & EXEC_FLAG_SMOOTH_QUANT) {
            smoothScGm.SetGlobalBuffer((__gm__ float *)gmMoeSmoothScales);
        }

        uint32_t proc = 0;
        for (uint32_t sg = 0; sg < moeExpertNumPerRank; ++sg) {
            for (uint32_t t = 0; t < tokPerCore; t++) {
                int32_t dstEid = expertIdsTensor_.GetValue(sTok + t);
                if (dstEid < 0)
                    continue;
                if ((uint32_t)(dstEid % moeExpertNumPerRank) != sg)
                    continue;

                uint32_t bI = (proc & 1) ? 0 : 1;
                int32_t evt = (proc & 1) ? 0 : 1;

                AscendC::LocalTensor<float> smSc = xInT[1 - bI].template ReinterpretCast<float>();
                AscendC::WaitFlag<AscendC::HardEvent::V_MTE2>(evt);
                AscendC::WaitFlag<AscendC::HardEvent::MTE3_MTE2>(evt);
                AscendC::DataCopy(xInT[bI], srcXGm[((sTok + t) / axisK) * tokenLength], tokenLength);
                if constexpr (EXEC_FLAG & EXEC_FLAG_SMOOTH_QUANT) {
                    AscendC::PipeBarrier<PIPE_MTE2>();
                    AscendC::DataCopy(smSc, smoothScGm[(uint32_t)dstEid * tokenLength], tokenLength);
                }
                AscendC::SetFlag<AscendC::HardEvent::MTE2_V>(evt);
                AscendC::WaitFlag<AscendC::HardEvent::MTE2_V>(evt);
                // CalExpandxIdx for expandIdx (global within-expert position).
                int32_t curPos = 0;
                CalExpandxIdx(dstEid, sTok + t, curPos, qUbOff);

                QuantToken(xInT[bI], smSc, yOutT[bI], qUbOff);
                AscendC::SetFlag<AscendC::HardEvent::V_MTE3>(evt);

                // Global within-expert position from CalExpandxIdx.
                // RemoteReadFunc adds earlier-rank offset to match.
                uint32_t wOff = (expBaseUb.GetValue((uint32_t)dstEid) + (uint32_t)curPos) * quantStride;

                // Buffer expandIdx (contiguous by local token index t).
                expIdxUb.SetValue(t, curPos);

                AscendC::WaitFlag<AscendC::HardEvent::S_MTE3>(0);
                AscendC::WaitFlag<AscendC::HardEvent::V_MTE3>(evt);
                AscendC::DataCopy(qOutGm[wOff], yOutT[bI], tokenLength);
                AscendC::PipeBarrier<PIPE_MTE3>();
                AscendC::DataCopyExtParams scaleWrCp = {1, QUANT_SCALE_SIZE, 0, 0, 0};
                AscendC::DataCopyPad(qOutGm[wOff + tokenLength], yOutT[bI][tokenLength], scaleWrCp);
        ...
        // Batch-flush expandIdx like expertCountTensor (contiguous range).
        AscendC::DataCopyExtParams expIdxParams = {1U, static_cast<uint32_t>(tokPerCore * sizeof(uint32_t)), 0U, 0U,
                                                   0U};
        AscendC::SetFlag<AscendC::HardEvent::S_MTE3>(0);
        AscendC::WaitFlag<AscendC::HardEvent::S_MTE3>(0);
        AscendC::DataCopyPad(expIdxGm[sTok], expIdxUb, expIdxParams);
    }

    // Poll completion flags from state window metadata slots.
    // PublishMetaFunc embeds metaCompleteFlag at totalExperts so flag and
    // counts travel in one MTE3 write (no HCCS reorder race).
    CATLASS_DEVICE void WaitMetaReady()
    {
        uint32_t nRanks = epRankSize;
        uint32_t slotDwords = metaSlotSize_ / sizeof(uint32_t);

        GM_ADDR flagsBase = GetZbalStateAddr(&zbalContext_, epRankId, dispatchSlotsOffset_);
        AscendC::GlobalTensor<uint32_t> flagsGm32;
        flagsGm32.SetGlobalBuffer((__gm__ uint32_t *)flagsBase);

        uint32_t expectedFlag = metaCompleteFlag;
        bool allReady = false;
        while (!allReady) {
            allReady = true;
            for (uint32_t r = 0; r < nRanks; r++) {
                ...
                AscendC::DataCacheCleanAndInvalid<uint32_t, AscendC::CacheLine::SINGLE_CACHE_LINE,
                                                  AscendC::DcciDst::CACHELINE_OUT>(
                    flagsGm32[r * slotDwords + totalExperts]);
                __asm__ __volatile__("");
                if (flagsGm32.GetValue(r * slotDwords + totalExperts) != expectedFlag) {
                    allReady = false;
                }
            }
            if (!allReady) {
    ...
    CATLASS_DEVICE void PublishMetaFunc(GM_ADDR gmEpSendCount, GM_ADDR gmQuantWs)
    {
        AscendC::PipeBarrier<PIPE_ALL>();
        AscendC::GlobalTensor<uint32_t> epSendGm;
        epSendGm.SetGlobalBuffer((__gm__ uint32_t *)gmEpSendCount);
        uint32_t countsDwords = totalExperts;
        uint32_t metaDwords = metaSlotSize_ / sizeof(uint32_t);
        AscendC::LocalTensor<uint32_t> cntUb = resource.ubBuf.template GetBufferByByte<uint32_t>(0);

        // Bulk-read gmEpSendCount via DataCopy (replaces per-element scalar loop).
        // Invalidate the cache-line range first so MTE2 sees the atomic-add results.
        {
            uint32_t nLines = (countsDwords * sizeof(uint32_t) + 63) / 64 + 1;
            for (uint32_t i = 0; i < nLines; i++) {
                __asm__ __volatile__("");
                AscendC::DataCacheCleanAndInvalid<uint32_t, AscendC::CacheLine::SINGLE_CACHE_LINE,
                                                  AscendC::DcciDst::CACHELINE_OUT>(epSendGm[i * 16]);
                __asm__ __volatile__("");
            }
        }
        AscendC::DataCopy(cntUb, epSendGm, countsDwords);
        AscendC::SetFlag<AscendC::HardEvent::MTE2_S>(0);
        AscendC::WaitFlag<AscendC::HardEvent::MTE2_S>(0);

        // Fill remaining slot fields: flag + GVA, then zero pad to metaDwords.
        cntUb.SetValue(totalExperts, metaCompleteFlag);
        {
            uint64_t qGva = reinterpret_cast<uint64_t>(gmQuantWs);
            cntUb.SetValue(totalExperts + 1, static_cast<uint32_t>(qGva & 0xFFFFFFFFULL));
            cntUb.SetValue(totalExperts + 2, static_cast<uint32_t>((qGva >> 32) & 0xFFFFFFFFULL));
        }
        for (uint32_t i = totalExperts + 3; i < metaDwords; i++) {
            cntUb.SetValue(i, (uint32_t)0);
        }
        AscendC::SetFlag<AscendC::HardEvent::S_MTE3>(0);
        AscendC::WaitFlag<AscendC::HardEvent::S_MTE3>(0);

        for (uint32_t r = 0; r < epRankSize; r++) {
            GM_ADDR slotAddr = GetZbalStateAddr(&zbalContext_, r, dispatchSlotsOffset_);
            AscendC::GlobalTensor<uint32_t> slotGm;
            slotGm.SetGlobalBuffer((__gm__ uint32_t *)(slotAddr + epRankId * metaSlotSize_));
            AscendC::DataCopy(slotGm, cntUb, metaDwords);
        }
        AscendC::PipeBarrier<PIPE_ALL>();
    }
    // Multi-core wrapper: partitions remote ranks so each AIV core writes
    // a subset of state windows in parallel (same rank-partition pattern as
    // RemoteReadFunc).  Barrier already handled by the caller.
    CATLASS_DEVICE void PublishMetaSlice(GM_ADDR gmEpSendCount, GM_ADDR gmQuantWs, uint32_t startRank, uint32_t endRank)
    {
        if (startRank >= epRankSize)
            return;
    ...
    CATLASS_DEVICE void RemoteReadFunc(GM_ADDR gmX1, GM_ADDR gmX1Scale, GM_ADDR gmEpSendCount)
    {
        // Partition remote ranks across recv cores.
        uint32_t recvCoreNumPerGroup = recvCoreNum;
        uint32_t recvRankNumPerCoreVal = epRankSize / recvCoreNumPerGroup;
        uint32_t remainderRankNum = epRankSize % recvCoreNumPerGroup;
        uint32_t recvCoreIdxInGroup = recvCoreIdx % recvCoreNumPerGroup;
        uint32_t startRank = recvRankNumPerCoreVal * recvCoreIdxInGroup;
        if (recvCoreIdxInGroup < remainderRankNum) {
            recvRankNumPerCoreVal += 1;
            startRank += recvCoreIdxInGroup;
        } else {
            startRank += remainderRankNum;
        }
        uint32_t endRank = startRank + recvRankNumPerCoreVal;

        // ALL recv cores notify AIC (AIC waits for recvCoreNum × vToCFlag per group).
        bool hasWork = (startRank < epRankSize);
        int64_t subUbOffset = 0;
        uint32_t metaSlotDwords = 0;
        uint32_t metaBytes = 0;
        AscendC::LocalTensor<uint32_t> metaCacheUb;
        AscendC::LocalTensor<int8_t> x1Ub;
        AscendC::LocalTensor<uint32_t> cumTotalUb;

        if (hasWork) {
            WaitMetaReady();
            AscendC::PipeBarrier<PIPE_ALL>();

            metaSlotDwords = metaSlotSize_ / sizeof(uint32_t);
            metaBytes = metaSlotSize_;

            // Read all ranks' metadata slots into UB (MTE2 DataCopyPad).
            metaCacheUb = resource.ubBuf.template GetBufferByByte<uint32_t>(subUbOffset);
            subUbOffset += CEIL_UP(epRankSize * metaSlotSize_);

            for (uint32_t r = 0; r < epRankSize; r++) {
                GM_ADDR metaSlot = GetZbalStateAddr(&zbalContext_, epRankId, dispatchSlotsOffset_ + r * metaSlotSize_);
                AscendC::GlobalTensor<uint32_t> metaGm;
                metaGm.SetGlobalBuffer((__gm__ uint32_t *)metaSlot);
                AscendC::DataCopyExtParams metaCp = {1, static_cast<uint32_t>(metaBytes), 0, 0, 0};
                AscendC::DataCopyPadExtParams<uint32_t> metaPad{false, 0U, 0U, 0U};
                AscendC::DataCopyPad(metaCacheUb[r * metaSlotDwords], metaGm, metaCp, metaPad);
            }
            AscendC::SetFlag<AscendC::HardEvent::MTE2_V>(0);
            AscendC::WaitFlag<AscendC::HardEvent::MTE2_V>(0);

            // VECIN-position read buffer: holds up to MAX_VECIN_TOKENS tokens
            // so we can batch multiple contiguous tokens in one HCCS read.
            {
                uint32_t vecInOff = VECIN_READ_OFFSET + UB_ALIGN;
                x1Ub =
                    AscendC::LocalTensor<int8_t>(AscendC::TPosition::VECIN, vecInOff, MAX_VECIN_TOKENS * quantStride);
            }

            // cumTotalUb[e] = total tokens for global expert e across all source ranks.
            cumTotalUb = resource.ubBuf.template GetBufferByByte<uint32_t>(subUbOffset);
            subUbOffset += CEIL_UP(totalExperts * sizeof(uint32_t));

            // Vector Add across all ranks (int32 counts via fp32 reinterpret).
            AscendC::LocalTensor<float> cumFp32 = cumTotalUb.template ReinterpretCast<float>();
            AscendC::LocalTensor<float> metaFp32 = metaCacheUb.template ReinterpretCast<float>();
            AscendC::Duplicate(cumFp32, 0.0f, totalExperts);
            AscendC::PipeBarrier<PIPE_V>();
            for (uint32_t r = 0; r < epRankSize; r++) {
                AscendC::Add(cumFp32, cumFp32, metaFp32[r * metaSlotDwords], totalExperts);
                AscendC::PipeBarrier<PIPE_V>();
            }
            AscendC::SetFlag<AscendC::HardEvent::V_S>(0);
            AscendC::WaitFlag<AscendC::HardEvent::V_S>(0);
        ...
        // Running startOffset accumulator per source rank: eliminates the
        // inner expert-sum loop in the per-group per-rank processing below.
        AscendC::LocalTensor<uint32_t> startOffUb;
        if (hasWork) {
            startOffUb = resource.ubBuf.template GetBufferByByte<uint32_t>(subUbOffset);
            subUbOffset += CEIL_UP(epRankSize * sizeof(uint32_t));
            uint32_t base = epRankId * moeExpertNumPerRank;
            for (uint32_t r = 0; r < epRankSize; r++) {
                uint32_t rowBase = r * metaSlotDwords;
                uint32_t acc = 0;
                for (uint32_t e = 0; e < base; e++) {
                    acc += metaCacheUb.GetValue(rowBase + e);
                }
                startOffUb.SetValue(r, acc);
            }
            AscendC::SetFlag<AscendC::HardEvent::V_S>(0);
            AscendC::WaitFlag<AscendC::HardEvent::V_S>(0);
        }

        uint32_t localX1Off = 0;

        for (uint32_t groupId = 0; groupId < localExpertNum; ++groupId) {
            uint32_t coreTokenCount = 0;

            if (hasWork) {
                uint32_t gEid = epRankId * moeExpertNumPerRank + groupId;
                uint32_t groupStartOffset = localX1Off;
                uint32_t tokenPos = 0;
                for (uint32_t r = 0; r < startRank; r++) {
                    tokenPos += metaCacheUb.GetValue(r * metaSlotDwords + gEid);
                }

                for (uint32_t r = startRank; r < endRank; r++) {
                    uint32_t rowBase = r * metaSlotDwords;
                    uint32_t tokenCount = metaCacheUb.GetValue(rowBase + gEid);
                    // startOffset from precomputed accumulator (avoids inner
                    // sum-over-experts loop).  Advance for next groupId.
                    uint32_t startOffset = startOffUb.GetValue(r);
                    startOffUb.SetValue(r, startOffset + tokenCount);

                    if (tokenCount == 0)
                        continue;

                    AscendC::GlobalTensor<int8_t> x1Gm;
                    x1Gm.SetGlobalBuffer((__gm__ int8_t *)gmX1 + (groupStartOffset + tokenPos) * tokenLength);
                    AscendC::GlobalTensor<float> x1ScaleGm;
                    x1ScaleGm.SetGlobalBuffer((__gm__ float *)gmX1Scale);
                    uint64_t srcQuantGva =
                        static_cast<uint64_t>(metaCacheUb.GetValue(rowBase + totalExperts + 1)) |
                        (static_cast<uint64_t>(metaCacheUb.GetValue(rowBase + totalExperts + 2)) << 32);

                    for (uint32_t t = 0; t < tokenCount;) {
                        // Batch contiguous tokens in one HCCS read.
                        uint32_t batchSize = tokenCount - t;
                        if (batchSize > MAX_VECIN_TOKENS)
                            batchSize = MAX_VECIN_TOKENS;

                        uint64_t byteOff = (startOffset + t) * (uint64_t)quantStride;
                        GM_ADDR remoteAddr = (GM_ADDR)((__gm__ uint8_t *)srcQuantGva + byteOff);

                        AscendC::GlobalTensor<int8_t> remoteGm;
                        remoteGm.SetGlobalBuffer((__gm__ int8_t *)remoteAddr);

                        {
                            AscendC::DataCopyExtParams rdCp = {1, batchSize * quantStride, 0, 0, 0};
                            AscendC::DataCopyPadExtParams<int8_t> rdPad{false, 0U, 0U, 0U};
                            AscendC::DataCopyPad(x1Ub, remoteGm, rdCp, rdPad);
                        }
                        AscendC::SetFlag<AscendC::HardEvent::MTE2_MTE3>(0);
                        AscendC::WaitFlag<AscendC::HardEvent::MTE2_MTE3>(0);

                        // Write each token in the batch to X1 / X1Scale.
                        // Construct explicit VECIN LocalTensor views at each
                        // token's offset — DataCopy via operator[] on a
                        // VECIN-position tensor produces incorrect results.
                        // Enqueue all MTE3 writes then fence once per batch.
                        for (uint32_t b = 0; b < batchSize; b++) {
                            uint32_t tokVOff = VECIN_READ_OFFSET + UB_ALIGN + b * quantStride;
                            AscendC::LocalTensor<int8_t> tokUb(AscendC::TPosition::VECIN, tokVOff, tokenLength);
                            AscendC::DataCopy(x1Gm[(t + b) * tokenLength], tokUb, tokenLength);

                            AscendC::LocalTensor<float> scaleUb(AscendC::TPosition::VECIN, tokVOff + tokenLength,
                                                                sizeof(float));
                            AscendC::DataCopyExtParams scaleParams = {1, sizeof(float), 0, 0, 0};
                            AscendC::DataCopyPad(x1ScaleGm[groupStartOffset + tokenPos + t + b], scaleUb, scaleParams);
                        }
                        AscendC::SetFlag<AscendC::HardEvent::MTE3_MTE2>(0);
                        AscendC::WaitFlag<AscendC::HardEvent::MTE3_MTE2>(0);
                        t += batchSize;
                    }
                    coreTokenCount += tokenCount;
                    tokenPos += tokenCount;
                }
            }

            // Notify AIC tokens ready for this expert group.
            AscendC::PipeBarrier<PIPE_ALL>();
            AscendC::LocalTensor<int32_t> notifyUb = resource.ubBuf.template GetBufferByByte<int32_t>(subUbOffset);
            notifyUb.SetValue(CV_FLAG_INDEX, vToCFlag);
            notifyUb.SetValue(GROUP_ID_INDEX, groupId);
            notifyUb.SetValue(SELF_COUNT_INDEX, coreTokenCount);
            AscendC::SetFlag<AscendC::HardEvent::S_MTE3>(0);

            AscendC::GlobalTensor<int32_t> grpTokenTensor;
            grpTokenTensor.SetGlobalBuffer((__gm__ int32_t *)(statusDataSpaceGm + GROUP_TOKEN_NUM_OFFSET));
            AscendC::WaitFlag<AscendC::HardEvent::S_MTE3>(0);
            AscendC::SetAtomicAdd<int32_t>();
            AscendC::DataCopy(grpTokenTensor[groupId * GROUP_INFO_SIZE], notifyUb, INT32_COUNT_PER_BLOCK);
            AscendC::SetAtomicNone();
            AscendC::PipeBarrier<PIPE_ALL>();

            if (hasWork) {
                uint32_t gEid = epRankId * moeExpertNumPerRank + groupId;
                localX1Off += cumTotalUb.GetValue(gEid);
            }
        }

        // Fill gmEpSendCount: cumulative prefix sum over (group, srcRank).
        if (hasWork && recvCoreIdxInGroup == 0) {
            AscendC::GlobalTensor<uint32_t> epSendGm;
            epSendGm.SetGlobalBuffer((__gm__ uint32_t *)gmEpSendCount);
            uint32_t nEntries = localExpertNum * epRankSize;
            AscendC::LocalTensor<uint32_t> cumUb = resource.ubBuf.template GetBufferByByte<uint32_t>(subUbOffset);

            uint32_t cum = 0;
            for (uint32_t g = 0; g < localExpertNum; g++) {
                uint32_t gEid = epRankId * moeExpertNumPerRank + g;
                for (uint32_t r = 0; r < epRankSize; r++) {
                    cum += metaCacheUb.GetValue(r * metaSlotDwords + gEid);
                    cumUb.SetValue(g * epRankSize + r, cum);
                }
            }

            AscendC::DataCopyExtParams epCp = {1, static_cast<uint32_t>(nEntries * sizeof(uint32_t)), 0, 0, 0};
            AscendC::SetFlag<AscendC::HardEvent::S_MTE3>(0);
            AscendC::WaitFlag<AscendC::HardEvent::S_MTE3>(0);
            AscendC::DataCopyPad(epSendGm, cumUb, epCp);
        }
    }
    ...
    CATLASS_DEVICE void operator()<AscendC::AIV>(Params const &params)
    {
        ...
        if constexpr (EXEC_FLAG & EXEC_FLAG_SHARED_EXPERT) {
            if (isShareQuantCore) {
                shareQuantCoreFunc((GM_ADDR)params.gmX, (GM_ADDR)params.gmShareSmoothScales, (GM_ADDR)params.gmShareX1,
                                   (GM_ADDR)params.gmShareX1Scale);
            }
        }
        // === Pull-mode dispatch: LocalQuant → PublishMeta → RemoteRead ===
        LocalQuantFunc((GM_ADDR)params.gmX, (GM_ADDR)params.gmexpertIds, (GM_ADDR)params.gmMoeSmoothScales,
                       (GM_ADDR)params.gmEpSendCount, (GM_ADDR)params.gmExpandIdx, (GM_ADDR)params.gmQuantWorkspace);
        icache_preload(8);
        AscendC::PipeBarrier<PIPE_ALL>();
        AscendC::SyncAll<true>();

        {
            uint32_t pubCoreNum = aivNum;
            uint32_t pubCoreIdx = aivIdx;
            uint32_t ranksPerCore = epRankSize / pubCoreNum;
            uint32_t rem = epRankSize % pubCoreNum;
            uint32_t startR = ranksPerCore * pubCoreIdx;
            if (pubCoreIdx < rem) {
                ranksPerCore += 1;
                startR += pubCoreIdx;
            } else {
            ...
            PublishMetaSlice((GM_ADDR)params.gmEpSendCount, (GM_ADDR)params.gmQuantWorkspace, startR,
                             startR + ranksPerCore);
        }
        AscendC::PipeBarrier<PIPE_ALL>();
        AscendC::SyncAll<true>();

        // RemoteRead (recv cores) and CompCore (send cores) run concurrently.
        // No SyncAll between them — CompCore polls groupTokenNum internally.
        if (isRecvCore) {
            RemoteReadFunc((GM_ADDR)params.ptrA, (GM_ADDR)params.ptrPerTokenScale, (GM_ADDR)params.gmEpSendCount);
        }

        // Step 4: Comp (SwiGLU epilogue) — writes to gmSwigluOut/gmShareSwigluOut
        ...
        if (isSendCore) {
            CompCoreFunc(params.ptrWorkspace, params.ptrShareScale, params.ptrScale,
                         ...
                         gmShareSwigluOutput, gmSwigluOutput, params.shareN, params.problemShape.n(),
                         params.problemShape.k(), params.layoutShareScale, params.layoutScale,
                         params.layoutPerTokenScale, true); // allTilesMode=true
        }
        ...
        if constexpr (EXEC_FLAG & EXEC_FLAG_SHARED_EXPERT) {
            ...
            quantParams = typename BlockQuant<ArchTag>::Params{(__gm__ float *)params.gmShareSwigluOut,
                                                               swigluLayout, // input: swiglu output
                                                               (__gm__ float *)params.gmShareX2Scale,
                                                               params.layoutDequantScale, // output: quant token scale
                                                               (__gm__ int8_t *)params.gmShareX2,
                                                               params.layoutShareOutput, // output: x2
                                                               quantRowOnce,
                                                               nOut};
        ...
        }
        {
            ...
            uint32_t lastIdx = localExpertNum * epRankSize - 1;
            __asm__ __volatile__("");
            AscendC::DataCacheCleanAndInvalid<int32_t, AscendC::CacheLine::SINGLE_CACHE_LINE,
                                              AscendC::DcciDst::CACHELINE_OUT>(sendCountsGlobal[lastIdx]);
            __asm__ __volatile__("");
            totalTokenCount = sendCountsGlobal.GetValue(lastIdx);
            AscendC::PipeBarrier<PIPE_ALL>();
...
private:
    ...
    struct AicWaitFunc1 {
        ...
        void operator()() const
        {
            if (checkSubBlock1Only) {
                // Only wait for send-core CompCore (subBlock 1).
                // recv-core flag (subBlock 0) is never incremented because
                // recv cores are busy with RemoteReadFunc.
                CheckSyncFlagSingle(flagAddr, idx, target, 1);
            } else {
    ...
    uint32_t quantStride{0};      // token stride in quant buffer: tokenLength + QUANT_SCALE_SIZE (no padding)
    uint32_t totalExperts{0};     // epRankSize * moeExpertNumPerRank
    uint32_t metaCompleteFlag{0}; // flipped per kernel call (0x3F800000 ↔ 0), used by PublishMetaFunc/WaitMetaReady
    // Pull-mode dispatch: dynamic metadata slot layout (computed in AivInitParams).
    uint64_t dispatchFlagsSize_{0};   // epRankSize * sizeof(uint32_t)
    uint64_t dispatchSlotsOffset_{0}; // DISPATCH_FLAGS_OFFSET + dispatchFlagsSize_
    uint64_t metaSlotSize_{0};        // CEIL_UP(totalExperts * sizeof(uint32_t)) = 32B-aligned  per-rank metadata slot
    ...
    // memory info
    ZbalCommContext zbalContext_;
    GM_ADDR statusDataSpaceGm;
    uint32_t stateOffset{0};
    uint64_t expertPerSizeOnWin{0};
    uint64_t winDataSizeOffset{0};
    uint64_t quantDataOffset_{0}; // dataState * totalQuantBytes for dispatch push
...
public:
    ...
    struct Params {
        ...
        Params(GemmCoord problemShape_, uint32_t problemCount_, GM_ADDR ptrGroupList_, GM_ADDR ptrA_,
            ...
            : problemShape(problemShape_), problemCount(problemCount_),
              ...
              ptrOutput(reinterpret_cast<__gm__ ElementOutput *>(ptrOutput_)), layoutOutput(layoutOutput_),
              ptrDequantScale(reinterpret_cast<__gm__ ElementDequantScale *>(ptrDequantScale_)),
              layoutDequantScale(layoutDequantScale_), ptrWorkspace(ptrWorkspace_), gmShareX1(gmShareX1_),
              gmShareX1Scale(gmShareX1Scale_), gmShareSwigluOut(gmShareSwigluOut_), gmShareX2(gmShareX2_),
              layoutShareOutput(layoutShareOutput_), gmShareX2Scale(gmShareX2Scale_), gmSwigluOut(gmSwigluOut_),
              bs(disGmmDeqSwigluQuantGmmDeqComInfo.bs), shareN(disGmmDeqSwigluQuantGmmDeqComInfo.shareGmm1HLen)
        {}
    };
    ...
    CATLASS_DEVICE void operator()<AscendC::AIC>(Params const &params)
    {
        ...
        gmA.SetGlobalBuffer(params.ptrA);
        // ZBAL: gmB was already set from params.ptrB in initialization.
        // Do NOT use gmBlistTensorDesc.GetDataPtr(0) — ptrB is raw data, not a ListTensorDesc.
        gmB.SetGlobalBuffer(params.ptrB);
    ...
    CATLASS_DEVICE void operator()<AscendC::AIV>(Params const &params)
    {
        ...
        auto ptrD = reinterpret_cast<__gm__ float *>(params.gmSwigluOut);
...

        {
            ...
            if constexpr (EXEC_FLAG & EXEC_FLAG_SHARED_EXPERT) {
                ...
                EpilogueParams epilogueParams{params.ptrShareScale,
                                              params.layoutShareScale,
                                              reinterpret_cast<__gm__ float *>(params.gmShareX1Scale),
                                              layoutPerTokenScale,
            ...
            // ZBAL: ptrScale is a raw data pointer (not a ListTensorDesc from CANN opbuild).
            // Only wrap as ListTensorDesc when TENSOR_LIST flag is set.
            __gm__ ElementScale *gmScalePtr;
            if constexpr (EXEC_FLAG & EXEC_FLAG_TENSOR_LIST) {
                gmScaleListTensor = AscendC::ListTensorDesc(reinterpret_cast<__gm__ void *>(params.ptrScale));
            } else {
                gmScalePtr = params.ptrScale;
            }
            for (uint32_t groupIdx = 0; groupIdx < params.problemCount; ++groupIdx) {
                uint32_t currentM = (groupIdx == 0) ? groupList.GetValue(groupIdx)
                                                    : (groupList.GetValue(groupIdx) - groupList.GetValue(groupIdx - 1));
        ...
        if constexpr (EXEC_FLAG & EXEC_FLAG_SHARED_EXPERT) {
            ...
            typename BlockQuant<ArchTag>::Params quantParams{reinterpret_cast<__gm__ float *>(params.gmShareSwigluOut),
                                                             swigluLayout,
...
#endif // FUSED_DEEP_MOE_GEMM_KERNEL_GROUPED_MATMUL_SLICE_M_PER_TOKEN_DEQUANT_SWIGLU_QUANT_MULTISTAGE_WORKSPACE_H
