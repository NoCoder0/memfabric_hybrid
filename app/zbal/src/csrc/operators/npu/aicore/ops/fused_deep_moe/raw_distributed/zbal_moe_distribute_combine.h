...
*/
#ifndef ZBAL_MOE_DISTRIBUTE_COMBINE_H
#define ZBAL_MOE_DISTRIBUTE_COMBINE_H
...
// Offset in state window for combine workspace GVA exchange.
// Each rank writes 8 bytes at COMBINE_GVA_OFFSET + myRankId * 8.
constexpr uint64_t COMBINE_GVA_OFFSET = 960 * 1024;
...
struct CombineCalcInfo {
    ...
    // GVA of each remote rank's combine workspace (lazy-loaded).
    uint64_t remoteCombineGva_[16];
    uint32_t gvaLoadedMask_{0};  // bit r = 1 when remoteCombineGva_[r] is valid
    GM_ADDR localCombineWs_{0};  // local combine workspace base
    int32_t gvaEpStateValue_{0}; // expected flag value for GVA ready check
};
...
public:
    ...
    __aicore__ inline void Init(GM_ADDR expandX, GM_ADDR expertIds, GM_ADDR expandIdx, GM_ADDR epSendCount,
                                ...
                                GM_ADDR workspaceGM, TPipe *pipe, __gm__ const FusedDeepMoeTilingData *tilingData,
                                ZbalCommContext *epCtx, ZbalCommContext *tpCtx, GM_ADDR combineWs = 0);
    __aicore__ inline void Process();
    __aicore__ inline void AllToAllSend();
    __aicore__ inline void ReducePermute();
    __aicore__ inline void ProcessCombine();
    __aicore__ inline void PublishCombineGva();
    __aicore__ inline void LoadRemoteGva(uint32_t rankId);
...
private:
    ...
    {
        if (domain == EP_DOMAIN) {
            uint64_t offset = expertLocalId * expertPerSizeOnWin_;
            if (static_cast<uint32_t>(rankId) == epRankId_) {
                return (GM_ADDR)(calcInfo_.localCombineWs_ + offset);
            }
            if (!(calcInfo_.gvaLoadedMask_ & (1u << rankId))) {
                LoadRemoteGva(static_cast<uint32_t>(rankId));
            }
            return (GM_ADDR)(calcInfo_.remoteCombineGva_[rankId] + offset);
        } else {
            uint64_t offset = rankId * OPT_RANK_OFFSET;
            return GetZbalDataAddr(calcInfo_.tpZbalContext_, rankId, offset);
        }
    }

    __aicore__ GM_ADDR GetWinStateAddrByRankId(const int32_t rankId, const uint8_t domain)
    {
        uint64_t offset = dataState_ * WIN_STATE_OFFSET;
        if (domain == EP_DOMAIN) {
            return GetZbalStateAddr(calcInfo_.epZbalContext_, rankId, offset);
        } else {
            return GetZbalStateAddr(calcInfo_.tpZbalContext_, rankId, offset);
        }
    }
    ...
    GM_ADDR epWindowGM_;
    GM_ADDR gmCombineWs_; // local combine workspace (replaces data window)
...
__aicore__ inline void CamMoeDistributeCombine<TemplateMC2TypeFunc>::Init(
    ...
    GM_ADDR xActiveMask, GM_ADDR XOut, GM_ADDR workspaceGM, TPipe *pipe,
    __gm__ const FusedDeepMoeTilingData *tilingData, ZbalCommContext *epCtx, ZbalCommContext *tpCtx, GM_ADDR combineWs)
{
    tpipe_ = pipe;
    ...
    // Store zbal contexts
    calcInfo_.epZbalContext_ = epCtx;
    calcInfo_.tpZbalContext_ = tpCtx;

    GlobalTensor<int32_t> selfDataStatusTensor;
    GM_ADDR statusDataSpaceGm = (GM_ADDR)epCtx->stateWindowBase;
    ...
    tpWorldSize_ = 1;
    tpRankId_ = 0; // when using tp, it should be tpRankId_ = tpCtx->myRankId;
    ...
    winDataSizeOffset_ = 0ULL; // no longer used (was data window ping-pong offset)
    gmCombineWs_ = combineWs;
    ...
    calcInfo_.localCombineWs_ = gmCombineWs_;
    calcInfo_.gvaEpStateValue_ = epStateValue_;
    calcInfo_.gvaLoadedMask_ = 0;
}
...
__aicore__ inline void CamMoeDistributeCombine<TemplateMC2TypeFunc>::ExpertAlltoAllDispatchCopyAdd()
{
    ...
    uint32_t curRankExpertNum = moeExpertPerRankNum_;
    DataCopyExtParams epSendCntParams{1U, static_cast<uint32_t>(moeSendNum_ * sizeof(uint32_t)), 0U, 0U, 0U};
...
CamMoeDistributeCombine<TemplateMC2TypeFunc>::CustomAdd(LocalTensor<ExpandXType> &dst, LocalTensor<ExpandXType> &src0,
                                                        LocalTensor<ExpandXType> &src1, uint32_t dataCnt)
{
    if constexpr (AscendC::IsSameType<ExpandXType, bfloat16_t>::value) {
...
template<TemplateMC2TypeClass>
__aicore__ inline void CamMoeDistributeCombine<TemplateMC2TypeFunc>::PublishCombineGva()
{
    // Write local workspace GVA + flag to all remote ranks' state windows.
    // readStateBuf_ is normally set up in BuffInit(), but this function
    // is called before BuffInit.  Allocate directly from the TPipe here.
    tpipe_->InitBuffer(readStateBuf_, 32);
    LocalTensor<int32_t> ub = readStateBuf_.Get<int32_t>();
    uint64_t myGva = reinterpret_cast<uint64_t>(gmCombineWs_);
    ub.SetValue(0, static_cast<uint32_t>(myGva & 0xFFFFFFFFULL));
    ub.SetValue(1, static_cast<uint32_t>((myGva >> 32) & 0xFFFFFFFFULL));
    ub.SetValue(2, epStateValue_); // flag
    for (uint32_t i = 3; i < 8; i++)
        ub.SetValue(i, 0); // pad to 32B
    SyncFunc<AscendC::HardEvent::S_MTE3>();

    for (uint32_t r = startRankId_; r < endRankId_; r++) {
        stateGM_ = GetZbalStateAddr(calcInfo_.epZbalContext_, r, COMBINE_GVA_OFFSET + epRankId_ * 32);
        rankStates_.SetGlobalBuffer((__gm__ int32_t *)stateGM_);
        DataCopy(rankStates_, ub, 8);
    }
    // Flush MTE3 writes so remote ranks can see our GVA.
    pipe_barrier(PIPE_ALL);
}

template<TemplateMC2TypeClass>
__aicore__ inline void CamMoeDistributeCombine<TemplateMC2TypeFunc>::LoadRemoteGva(uint32_t rankId)
{
    if (calcInfo_.gvaLoadedMask_ & (1u << rankId)) {
        return;
    }

    // Read GVA flag from the local state window.
    GM_ADDR gvaBase = GetZbalStateAddr(calcInfo_.epZbalContext_, epRankId_, COMBINE_GVA_OFFSET);
    AscendC::GlobalTensor<uint32_t> gvaGm;
    gvaGm.SetGlobalBuffer((__gm__ uint32_t *)gvaBase);

    while (!(calcInfo_.gvaLoadedMask_ & (1u << rankId))) {
        __asm__ __volatile__("");
        ...
        __asm__ __volatile__("");
        if (gvaGm.GetValue(rankId * 8 + 2) == static_cast<uint32_t>(calcInfo_.gvaEpStateValue_)) {
            uint32_t lo = gvaGm.GetValue(rankId * 8);
            uint32_t hi = gvaGm.GetValue(rankId * 8 + 1);
            calcInfo_.remoteCombineGva_[rankId] = static_cast<uint64_t>(lo) | (static_cast<uint64_t>(hi) << 32);
            calcInfo_.gvaLoadedMask_ |= (1u << rankId);
        } else {
            SPIN_WAIT_CYCLES();
        }
    }
...
#endif // ZBAL_MOE_DISTRIBUTE_COMBINE_H
