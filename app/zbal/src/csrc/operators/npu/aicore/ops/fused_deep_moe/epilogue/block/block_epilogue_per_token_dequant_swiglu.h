...
*/
#ifndef FUSED_DEEP_MOE_EPILOGUE_BLOCK_BLOCK_EPILOGUE_PER_TOKEN_DEQUANT_SWIGLU_H
#define FUSED_DEEP_MOE_EPILOGUE_BLOCK_BLOCK_EPILOGUE_PER_TOKEN_DEQUANT_SWIGLU_H
...
public:
                    ...
                    bool allTilesMode = false)
    {
        ...
        // When CompCore runs on only one subBlock (send cores), process ALL
        // tiles within the block instead of the usual subBlock stride split.
        uint32_t tStart = allTilesMode ? 0u : subblockIdx;
        uint32_t tStride = allTilesMode ? 1u : subblockNum;
        for (uint32_t loopIdx = tStart; loopIdx < tileLoops; loopIdx += tStride) {
...
#endif // FUSED_DEEP_MOE_EPILOGUE_BLOCK_BLOCK_EPILOGUE_PER_TOKEN_DEQUANT_SWIGLU_H
