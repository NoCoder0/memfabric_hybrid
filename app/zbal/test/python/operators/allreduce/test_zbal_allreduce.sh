...
if [[ $nnodes -eq 1 ]]; then
    if [[ ${ZBAL_ENABLE_PERF_TEST} = "1" ]]; then
        torchrun --nproc-per-node $WORLD_SIZE --master-port 8778 ${CURRENT_DIR}/test_zbal_allreduce.py hccl --case_num $CASE_NUM --case_list $CASE_LIST --data_op_type $DATA_OP_TYPE
    fi
