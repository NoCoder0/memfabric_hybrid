...
def gen_random_data(size, dtype):
    data = torch.empty(size).uniform_(0.0, 10.0).to(dtype)
    return data


def golden_generate(data_len, rank_size, data_type, current_dir):
    golden_dir = f"allreduce_{data_len}_{rank_size}"
    cmd = f"mkdir -p {current_dir}/golden/{golden_dir}"
    os.system(cmd)

    input_gm = [None] * rank_size

    for i in range(rank_size):
        input_gm[i] = gen_random_data((data_len), dtype=data_type)
        with open(f"{current_dir}/golden/{golden_dir}/input_gm_{i}.pkl", 'wb') as f:
            pickle.dump(input_gm[i], f)

    logger.info(f"{data_len} allreduce golden generate success !")
