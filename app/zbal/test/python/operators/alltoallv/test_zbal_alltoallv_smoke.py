...
def _assemble_golden(case_id, world_size, rank, cur_input_splits, cur_output_splits,
                     output_shape, data_type, tensor_dtype, current_dir):
    parts = []
    ...
    if not parts:
        return torch.zeros(0, output_shape[1], dtype=tensor_dtype).npu() if len(
            output_shape) == 2 else torch.tensor([], dtype=tensor_dtype).npu()
...
def test_alltoallv(dist_type, data_op_type):
    ...
    try:
        for i in range(len(case_index)):
            ...
            golden = None
            if dist_type == 'zbal':
                golden = _assemble_golden(case_id, world_size, global_rank,
                                          splits_in[i], splits_out[i],
                                          shapes_out[i][global_rank],
                                          data_type, tensor_dtype, current_dir)

            in_shape = shapes_in[i][global_rank]
            gd = f"alltoallv_{case_id}_{world_size}_{global_rank}"
            tensor_input = _load_tensor(
                f"{current_dir}/golden/{gd}/input_gm_{global_rank}.bin",
                in_shape, data_type, tensor_dtype)

            for j in range(REPEAT_TIMES):
                try:
                    tensor_output = torch.zeros(shapes_out[i][global_rank],
                                                dtype=tensor_input.dtype,
                                                device=tensor_input.device)
                    dist.all_to_all_single(tensor_output, tensor_input,
                                          splits_out[i][global_rank],
                                          splits_in[i][global_rank])
