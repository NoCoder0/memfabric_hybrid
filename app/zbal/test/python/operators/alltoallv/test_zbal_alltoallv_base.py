...
TYPE_MAP = {"int": np.int32, "int32_t": np.int32, "float16_t": np.float16,
            "float": np.float32, "bfloat16_t": np.float16}
TORCH_TYPE_MAP = {"int": torch.int32, "int32_t": torch.int32, "float16_t": torch.float16,
                  "float": torch.float32, "bfloat16_t": torch.bfloat16}
REPEAT_TIMES = 20
HIDDEN_SIZE = 1024
...
def init_group(dist_type, world_size, data_op_type=0):
    ...
    if dist_type == "zbal":
        ...
        zbal_init(world_size, int(os.environ["LOCAL_RANK"]), rank,
                  4 * 1024 * 1024 * 1024, data_op_type=data_op_type)
