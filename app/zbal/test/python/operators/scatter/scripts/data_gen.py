...
def golden_generate(data_len, rank_size, hidden_size, data_type, current_dir):
    ...
    golden_dir = f"scatter_{rank_size}_{input_rows}_{hidden_size}"
    cmd = f"mkdir -p {current_dir}/golden/{golden_dir}"
    os.system(cmd)

    input_gm = []
    output_gm = [np.zeros((input_rows // rank_size, hidden_size), dtype=data_type) for _ in range(rank_size)]
    root = 0
    # generate all input
