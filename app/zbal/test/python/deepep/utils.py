...
def register_zbal(local_rank, rank, world_size, ip, port):
    ...
    local_mem_size = 50 * 1024 * 1024 * 1024
    ...
    logging.info(f"rank[{rank}] register zbal, {meta_addr=} {npu_tensor=}")
...
def init_dist(local_rank: int, num_local_ranks: int):
    ...
    register_zbal(local_rank, global_rank, world_size, ip, port)
...
def bench_kineto(
    ...
    # which will cause problems with the json format,
    # so here we use a random file name instead of creating a temporary file
