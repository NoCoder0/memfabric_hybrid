...
`local_dram_size` 和 `local_hbm_size` 才决定各自实际分配和导出的 slice。该验证固定
`max_dram_size=POOL_BYTES`；Ascend 950 的 HBM VMM 要求 `max_hbm_size` 按 GiB 对齐，因此设为
`HBM_GVA_MAX_BYTES`（1 GiB）。Host 仅提供 DRAM，NPU 仅提供 HBM，实际 HBM pool 仍为 `POOL_BYTES`。
...
4. `bm.create2(local_dram_size=POOL_BYTES, max_dram_size=POOL_BYTES,
   local_hbm_size=0, max_hbm_size=HBM_GVA_MAX_BYTES, data_op_type=HOST_DEVICE_URMA, enable_56bits_gva=False)`；
...
4. `bm.create2(local_dram_size=0, max_dram_size=POOL_BYTES,
   local_hbm_size=POOL_BYTES, max_hbm_size=HBM_GVA_MAX_BYTES, data_op_type=HOST_DEVICE_URMA)` 并 `join()`；
